...
# ==================== 1. 队名映射与翻译系统 ====================
# 这里的映射表用于将中文队名精准转化为 API-Football 能识别的英文，从而抓取真实进球数/近况
TEAM_NAME_MAPPING = {
...
def scrape_wencai_jczq(date=None):
    """
    直连 edu.wencaivip.cn 核心接口。
    数据密度：让球盘口、初赔、赔率异动、伤停利空、专家推介、基本面研报。
    """
    url = "https://edu.wencaivip.cn/api/v1.reference/matches"
    print(f"  🌐 正在接入深度情报网络: {url}")
    ms = []
    ...
    try:
        ...
        print(f"  ✅ 发现 {len(match_list)} 场有效竞彩赛事数据！")
        ...
        for item in match_list:
            try:
                ...
                # B. 赔率异动监测 (change 字段: 1=升, -1=降, 0=稳)
                chg = item.get("change", {})
                w_c = chg.get("win", 0)
                l_c = chg.get("lose", 0)
                ...
                # D. 深度文本情报提取 (核心进化)
                expert_intro = item.get("intro", "") # 专家推介语
...
def collect_all(date=None):
    ...
    for i, m in enumerate(matches):
        print(f"  [{i+1}/{len(matches)}] 深度透视: {m['home_team']} vs {m['away_team']}")
    ...
    return {"date": date, "matches": matches, "odds": fetch_odds(), "fetch_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
