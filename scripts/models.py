...
class SmartMoneyDetector:
    def analyze(self, model_h, model_a, match_odds):
        if not match_odds or not match_odds.get("bookmakers"): return {"home_rlm_adj": 0, "away_rlm_adj": 0, "signal": "正常"}
        ho, ao = [], []
        for bk in match_odds["bookmakers"]:
            h2h = bk.get("markets", {}).get("h2h", {})
            if "Home" in h2h: ho.append(h2h["Home"])
        ...
        diff_h = (1 / (sum(ho)/len(ho)) * 100) - model_h; diff_a = (1 / (sum(ao)/len(ao)) * 100) - model_a
        signal = "⚠️ 机构防范主队" if diff_h > 12 else "⚠️ 机构防范客队" if diff_a > 12 else "🚨 庄家诱盘主队" if diff_h < -12 else "🚨 庄家诱盘客队" if diff_a < -12 else "正常"
...
class BivariatePoissonModel:
    def predict(self, home_gf, home_ga, away_gf, away_ga):
        he = max(0.4, min(float(home_gf or 1.3) * 1.05, 3.5)); ae = max(0.3, min(float(away_gf or 1.1) * 0.95, 3.0))
        rho = 0.2 if abs(he - ae) < 0.4 else 0.05 
        hw, dr, aw = 0.0, 0.0, 0.0; scores = []
        ...
        return {"home_win": round(hw/t*100, 1), "draw": round(dr/t*100, 1), "away_win": round(aw/t*100, 1), "predicted_score": f"{scores[0][0]}-{scores[0][1]}"}

class PaceTotalGoalsModel:
    def predict(self, hs, ast):
        exp_total = float(hs.get("avg_goals_for", 1.3)) * float(ast.get("avg_goals_against", 1.3)) + float(ast.get("avg_goals_for", 1.1)) * float(hs.get("avg_goals_against", 1.1))
...
class PoissonModel:
    def predict(self, home_gf, home_ga, away_gf, away_ga, league_avg=1.35):
        ...
        def pmf(k, l): return (l**k) * math.exp(-l) / math.factorial(k)
        hw, dr, aw, bt = 0.0, 0.0, 0.0, 0.0; scores = []
        for i in range(7):
            for j in range(7):
                p = pmf(i, he) * pmf(j, ae)
                if i > j: hw += p
                ...
                scores.append((i, j, p))
        scores.sort(key=lambda x: x[2], reverse=True); t = hw + dr + aw
        return {"home_win": round(hw/t*100, 1), "draw": round(dr/t*100, 1), "away_win": round(aw/t*100, 1), "predicted_score": f"{scores[0][0]}-{scores[0][1]}", "btts": round(bt*100, 1)}

class DixonColesModel:
    def predict(self, home_gf, home_ga, away_gf, away_ga):
        he = max(0.3, min(float(home_gf or 1.3) * 1.05, 4.0)); ae = max(0.2, min(float(away_gf or 1.1) * 0.95, 3.5))
...
class BradleyTerryModel:
    def predict(self, hw, ht, aw, at):
        hp = max(0.1, int(hw or 5) / max(1, int(ht or 15))); ap = max(0.1, int(aw or 5) / max(1, int(at or 15)))
        h_str = hp / (hp + ap) * 1.10; a_str = ap / (hp + ap) * 0.90; dr = 0.25
        h = h_str * (1 - dr); a = a_str * (1 - dr); t = h + dr + a
        return {"home_win": round(h/t*100, 1), "draw": round(dr/t*100, 1), "away_win": round(a/t*100, 1)}

class BayesianModel:
    def predict(self, hw, hd, hl, aw, ad, al):
        ph = 1.2 + int(hw or 5)*1.0 + int(al or 3)*0.6; pd = 1.0 + int(hd or 3)*0.8 + int(ad or 3)*0.8; pa = 0.8 + int(aw or 5)*1.0 + int(hl or 3)*0.6; t = ph + pd + pa
...
class EnsemblePredictor:
    def __init__(self):
        ...
        print("[Models] 正在初始化量化分析引擎(精简 8 核心模型矩阵)...")
    ...
    def predict(self, match, odds_data=None):
        ...
        # 重新分配权重（因为去掉了 3 个模型，总权重需补齐）
        w = {
            "poisson": 0.15, "bivariate": 0.12, "dixon": 0.12, 
            "bt": 0.10, "bayes": 0.08, 
            "rf": 0.20, "nn": 0.13, "lr": 0.10
        }
