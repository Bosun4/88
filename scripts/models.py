...
class PoissonModel:
    ...
    def predict(self,home_gf,home_ga,away_gf,away_ga,league_avg=1.35):
        ...
        ha=home_gf/league_avg;aa=away_gf/league_avg;hd=home_ga/league_avg;ad=away_ga/league_avg
        ...
        return{"home_win":round(hw*100,1),"draw":round(dr*100,1),"away_win":round(aw*100,1),"predicted_score":"%d-%d"%(scores[0][0],scores[0][1]),"home_xg":round(he,2),"away_xg":round(ae,2),"over_2_5":round(o25*100,1),"btts":round(bt*100,1),"top_scores":[{"score":"%d-%d"%(s[0],s[1]),"prob":round(s[2]*100,1)}for s in scores[:5]]}
...
class EloModel:
    """ELO评分系统"""
    def __init__(self):self.ratings=defaultdict(lambda:1500);self.k=32
    ...
    def predict(self,h,a):
        rh=self.ratings[h]+65;ra=self.ratings[a];eh=1/(1+10**((ra-rh)/400))
...
class BradleyTerryModel:
    ...
    def predict(self,home_wins,home_total,away_wins,away_total):
        try:
            hw=int(home_wins or 0);ht=int(home_total or 1);aw=int(away_wins or 0);at=int(away_total or 1)
...
class EnsemblePredictor:
    ...
    def predict(self,match,odds_data=None):
        ...
        cf=min(92,max(30,cf))
        return{"home_win_pct":hp,"draw_pct":dp,"away_win_pct":ap,"predicted_score":poi["predicted_score"],"confidence":cf,"poisson":poi,"dixon_coles":dc,"elo":elo,"bradley_terry":bt,"monte_carlo":mc,"bayesian":bay,"random_forest":rf,"gradient_boost":gb,"neural_net":nn,"logistic":lr,"home_form":hf,"away_form":af,"odds":oa,"over_2_5":poi["over_2_5"],"btts":poi["btts"],"top_scores":mc.get("top_scores",poi.get("top_scores",[])),"model_weights":w,"model_consensus":consensus,"total_models":len(models)}
