...
“””
解析实际让球值,约定:主让为正,客让为负 (内部语义)

```
竞彩原始格式:give_ball 字段,负号表示主让
  give_ball = "-1"     → 主让1球 → 返回 +1.0
  give_ball = "+1"     → 主受让1球 → 返回 -1.0
  give_ball = "-0.5/1" → 主让 0.5/1 → 返回 +0.75
  give_ball = "-2球"   → 主让2球 → 返回 +2.0
"""
...
# 处理分数盘 "1/1.5", "-0.5/1", "+0.5/1"
if "/" in s:
    parts = s.split("/", 1)
    try:
        left = _f(parts[0].strip(), 0.0)
        right_raw = parts[1].strip()
        # "-0.5/1" 的右边1默认跟左边符号
        if not right_raw.startswith(("-", "+")):
            if left < 0:
                right_raw = "-" + right_raw
            elif left > 0:
                right_raw = "+" + right_raw
        right = _f(right_raw, 0.0)
        val = (left + right) / 2.0
        return -val  # 翻转 竞彩负号=主让 → 内部主让为正
    except:
        pass
...
“””
让球深度 vs 理论让球差异陷阱 (v18.1 修复版)

```
语义约定(重要):
  主让为正(give_ball="-1" → 内部 actual=+1.0)
  "让球深度" = 强方让弱方的球数 = abs(让球值)

v18.1 核心修复:
  按绝对值深度比较,并与基本面一致性检验
  - 若欧赔强方=基本面强方=让球强方,让球只是正常的"深度校准"
  - 若三者不一致,才真的是诱盘
"""
sp_h = _f(match_obj.get("sp_home", match_obj.get("win", 0)))
sp_a = _f(match_obj.get("sp_away", match_obj.get("lose", 0)))
...
odds_weak = "away" if odds_strong == "home" else "home"

# 按欧赔强方视角,计算理论/实际的"强方让球深度"
# 正值=强方让球(强方让弱方),负值=强方受让
if odds_strong == "home":
    theo_depth_signed = theoretical   # 主强,理论>=0 正常
    actual_depth_signed = actual      # 主让正常
else:
    theo_depth_signed = -theoretical  # 客强,理论应<=0,翻转让其为正
    actual_depth_signed = -actual

diff_depth = actual_depth_signed - theo_depth_signed

if abs(diff_depth) < 0.5:
    return None
...
if fund_h["total"] >= 3 and fund_a["total"] >= 3:
    ...
    if odds_strong == "home" and fund_diff >= 25:
        return None  # 让球只是正常校准
    # 欧赔指客强且基本面大幅支持客强
    elif odds_strong == "away" and fund_diff <= -25:
        return None

strong_side = odds_strong
weak_side = odds_weak
strong_cn = "主" if strong_side == "home" else "客"

if diff_depth >= 0.5:
    # T2: 让得更深 → 强方真强
    severity = 2 if abs(diff_depth) < 1.0 else 3
    boost = ["2-0", "3-0", "2-1", "3-1"] if strong_side == "home" else ["0-2", "0-3", "1-2", "1-3"]
    if abs(diff_depth) < 1.0:
        boost = boost[:2]
    return {
        "trap": "T2_HANDICAP_DEEPER",
        "description": f"让球偏深:{strong_cn}强方理论让{theo_depth_signed:+.2f} vs 实际让{actual_depth_signed:+.2f}深{diff_depth:+.2f} ({strong_cn}真强)",
        "severity": severity,
        "direction_adjust": {
            strong_side: +1.2 * min(2.0, abs(diff_depth)),
            weak_side: -0.5,
            "draw": -0.4
        },
        "score_multipliers": {},
    ...
    # T3: 让得更浅 → 强方没那么强
    severity = 2 if abs(diff_depth) < 1.0 else 3
    boost = ["1-1", "0-1", "1-2"] if strong_side == "home" else ["1-1", "1-0", "2-1"]
    return {
        "trap": "T3_HANDICAP_SHALLOWER",
        "description": f"让球偏浅:{strong_cn}强方理论让{theo_depth_signed:+.2f} vs 实际让{actual_depth_signed:+.2f}浅{diff_depth:+.2f} ({strong_cn}没那么强)",
        "severity": severity,
        "direction_adjust": {
            strong_side: -1.0 * min(2.0, abs(diff_depth)),
            weak_side: +0.6 * min(2.0, abs(diff_depth)),
            "draw": +0.8 * min(1.5, abs(diff_depth))
        },
        "score_multipliers": {},
        "boost_scores": boost,
    }
...
# 从多个可能字段聚合联赛/赛事文本
league_candidates = [
    match_obj.get("league", ""),
    match_obj.get("cup", ""),
    match_obj.get("match_league", ""),
    match_obj.get("match_type", ""),
    match_obj.get("sub_league_name", ""),
    match_obj.get("league_name", ""),
    match_obj.get("tournament", ""),
    match_obj.get("match_num", ""),  # 兼容号码里含"杯"
    match_obj.get("name", ""),
]
# 情报文本也可能含"杯赛/淘汰赛"字样
info = match_obj.get("information", {}) or {}
if isinstance(info, dict):
    for k in ["match_info", "cup_info", "notes"]:
        v = info.get(k, "")
        if v:
            league_candidates.append(str(v)[:200])
points = match_obj.get("points", {}) or {}
if isinstance(points, dict):
    league_candidates.append(str(points.get("match_points", ""))[:200])

league_text = " ".join(str(x) for x in league_candidates if x)

cup_keywords = ["杯", "淘汰", "决赛", "半决赛", "四分之一", "欧冠", "欧联",
               "国王杯", "足总杯", "联赛杯", "意大利杯", "法国杯", "德国杯",
               "西班牙超级", "欧罗巴", "Cup", "cup"]
is_cup = any(kw in league_text for kw in cup_keywords)
...
return {
    ...
    "severity": severity,
    "direction_adjust": {"draw": +1.8, "home": -0.6, "away": -0.6},
    "score_multipliers": {
        # 单边大比分严厉压制
        "3-1": 0.4, "1-3": 0.4, "3-0": 0.3, "0-3": 0.3,
        "2-0": 0.6, "0-2": 0.6, "3-2": 0.3, "2-3": 0.3,
        # 客胜/主胜常规比分适度压制(让历史僵局压得过 Sharp 信号)
        "1-2": 0.65, "2-1": 0.65,
    },
    "boost_scores": ["1-1", "0-0", "1-0", "0-1"],
    "suppress_other_traps": True,  # 标记:T15触发时,T16/T4/T5等单边陷阱应降权
}
```
...
shin: Dict) -> Optional[Dict]:
“””
Sharp 流向 + 该方坏消息爆炸 = Sharp 是诱饵(庄家造热收韭菜)
...
v18.1 修复:
  原版过度偏平局,实际真实流向应该是 Sharp 的反方
  此时 Sharp 是庄家故意放出的诱饵,真实资金其实在反方
  修改为: 反指 Sharp 方向,但平局仅轻微加成
"""
sigs_str = " ".join(str(s) for s in smart_signals)
...
# 反方向 = Sharp 的反方
reverse_dir = "away" if sharp_dir == "home" else "home"

return {
    "trap": "T16_SHARP_BADNEWS_CONFLICT",
    "description": f"Sharp({sharp_dir})+该方坏消息爆炸 → Sharp是诱饵,真实流向反方",
    "severity": 2,
    "direction_adjust": {
        sharp_dir: -1.2,   # 大幅压 Sharp 方向
        reverse_dir: +1.5,  # 加权反方
        "draw": +0.5,       # 平局轻微加成
    },
    "score_multipliers": {},
    "boost_scores": ["1-1", "1-0", "0-1"],
    "downgrade_sharp_trust": 0.1,  # 严重降 Sharp 权重
}
```
...
# 规则6: T15 (历史僵局) 存在时,T16 (Sharp对冲) 的单边反指效应要大幅减弱
# 因为历史僵局已证明两队互克,Sharp 反方向的信号价值被稀释
has_t15 = any(t.get("trap") == "T15_HISTORICAL_DEADLOCK" for t in traps)
if has_t15:
    for t in traps:
        if t.get("trap") == "T16_SHARP_BADNEWS_CONFLICT":
            # 削弱 T16 的反方向加成,保留其"降Sharp权重"作用
            adj = t.get("direction_adjust", {})
            for k in adj:
                adj[k] *= 0.4  # 全部方向调整打4折
            t["description"] += " [T15叠加,调整打4折]"

# 规则7: T3(让球偏浅压强方)与 T16(Sharp诱饵反指)同场时,如果 T16 反指的方向
# 正好是 T3 压制的强方,说明让球浅其实是庄家"故意不开深"的诱盘策略,T3 应降权
t3_r = next((t for t in traps if t.get("trap") == "T3_HANDICAP_SHALLOWER"), None)
t16_r = next((t for t in traps if t.get("trap") == "T16_SHARP_BADNEWS_CONFLICT"), None)
if t3_r and t16_r:
    # T3 压制的是"欧赔强方"(description里提到的 strong_cn)
    # T16 反指 Sharp 的反方向,反方向 = Sharp 的反方 = 应该是欧赔强方(如果Sharp是诱饵)
    # 这时 T3 的压制方向与 T16 支持的方向冲突 → T3 应该降权
    t3_adj = t3_r.get("direction_adjust", {})
    t16_adj = t16_r.get("direction_adjust", {})
    # 找 T3 压谁最狠(最负值),T16 挺谁最狠(最正值)
    t3_pressed = min(t3_adj, key=t3_adj.get)
    t16_boosted = max(t16_adj, key=t16_adj.get)
    if t3_pressed == t16_boosted and t3_pressed != "draw":
        # 二者冲突:T3 压 A,T16 挺 A → T3 降权
        for k in t3_adj:
            t3_adj[k] *= 0.25  # T3 调整打 2.5 折
        t3_r["description"] += " [T16反指冲突,T3调整打2.5折]"
        # 同时 T3 的 score_multipliers 也放松
        mults = t3_r.get("score_multipliers", {})
        for k in mults:
            mults[k] = min(1.0, mults[k] * 1.8)
...
def _normalize_score_str(s: Any) -> str:
“””
归一化比分字符串:
- 全角字符转半角
- 去除空格、冒号换为连字符
- 保留”胜其他/平其他/负其他”的中文标签
“””
if s is None:
return “”
ss = str(s).strip()
...
# 保留”XX其他”标签
for lab in (“胜其他”, “平其他”, “负其他”):
if ss.endswith(lab) or ss.startswith(lab):
return lab
return ss

def _safe_int(v, default=0):
“”“安全整数转换,处理 ‘85%’ / 85.5 / None”””
if v is None:
return default
try:
...
“”“确保返回列表 — 处理 AI 返回字符串/None/dict 的情况”””
if v is None:
return []
...
try:
    if s is None:
        return None, None
    s_str = str(s).strip()
    if not s_str or s_str in ("-", "N/A", "弃权", ""):
        return None, None
    
    # 全角到半角统一
    s_str = s_str.replace("：", "-").replace(":", "-").replace("–", "-").replace("—", "-")
    s_str = s_str.replace(" ", "").replace("\u3000", "")  # 去空格和全角空格
    
    # 严格识别"胜其他"类字符串,防止"客胜其他"之类误触
    # 先排除带"客胜"/"主胜"/"平局"前缀的
    if s_str.startswith("胜其他") or s_str == "胜其他场" or "→胜其他" in s_str:
        return 9, 0
    if s_str.startswith("平其他") or s_str == "平其他场" or "→平其他" in s_str:
        return 9, 9
    if s_str.startswith("负其他") or s_str == "负其他场" or "→负其他" in s_str:
        return 0, 9
    # 尾部包含"其他"的兜底(常见于 AI 输出"胜其他"在尾)
    if s_str.endswith("胜其他"):
        return 9, 0
    if s_str.endswith("平其他"):
        return 9, 9
    if s_str.endswith("负其他"):
        return 0, 9
    
    # 纯方向词不算比分
    if s_str in ["主胜", "客胜", "平局", "主", "客", "平", "胜", "负"]:
        return None, None
    ...
    h_val = int(p[0].strip())
    a_val = int(p[1].strip())
    if h_val < 0 or a_val < 0 or h_val > 99 or a_val > 99:
        return None, None
    return h_val, a_val
except:
    return None, None
...
for name, r in ai_responses.items():
    ...
    # top1 解析 - 用 is None 判断,而非 not(因为 0-0 会返回 (0,0),not 0 是True!)
    sc = sc_raw
    h_check, _ = _parse_score(sc)
    if h_check is None:
        # top1 无法解析,尝试从 top3 读
        if top3 and isinstance(top3, list) and len(top3) > 0 and isinstance(top3[0], dict):
            sc = top3[0].get("score", "") or top3[0].get("比分", "") or top3[0].get("scoreline", "")
    ...
    # 记录具体比分投票(归一化字符串)
    sc_clean = _normalize_score_str(sc)
    ...
    if isinstance(top3, list):
        for rank, t in enumerate(top3[1:3], 2):
            if not isinstance(t, dict):
                continue
            sc2_raw = t.get("score", "") or t.get("比分", "") or t.get("scoreline", "")
            sc2 = _normalize_score_str(sc2_raw)
...
```
v18.1 加固:
- Gemini SAFETY/MAX_TOKENS 截断兜底
- content=None 时强制从 reasoning_content 等字段挖
- 嵌套 data 字段探测 (如 OpenAI Assistants v2 结构)
"""
raw_text = ""
try:
    # ---- Gemini 响应 ----
                        ...
                        for p in parts:
                            if isinstance(p, dict) and p.get("text"):
                                ...
                                if t and len(t) > len(raw_text):
                                    raw_text = t
                if finish == "MAX_TOKENS" and raw_text:
                    print(f"    ⚠️ Gemini MAX_TOKENS 截断,已拿部分")
        # Gemini 兜底:直接扫字符串
        if not raw_text:
            full_str = json.dumps(data, ensure_ascii=False)
            m_match = re.search(r'\[\s*\{\s*\\?"match\\?"', full_str)
            if m_match:
                raw_text = _extract_json_from_dump(full_str, m_match.start())
    # ---- OpenAI / Claude / 其他标准 ----
    else:
        # 先检测 content=None 的 proxy bug,并预先准备挖 reasoning
        content_null_but_tokens = False
        if data.get("choices"):
            msg = data["choices"][0].get("message", {})
            if isinstance(msg, dict):
                if msg.get("content") is None and data.get("usage", {}).get("completion_tokens", 0) > 100:
                    content_null_but_tokens = True
                    print(f"    🚨 [proxy bug] {ai_name.upper()} content=null 但消耗了 {data['usage']['completion_tokens']} token — 尝试从 reasoning 挖")
                ...
                # 优先级 3: content=null 特殊处理 — 从 reasoning 字段挖 JSON
                # 注意:只有当 content=null 且代币花了时,才挖 reasoning(O1 类模型把答案放这里)
                if not raw_text and content_null_but_tokens:
                    for field in ["reasoning_content", "reasoning", "reasoning_text",
                                 "thinking", "thoughts", "chain_of_thought"]:
                        v = msg.get(field, "")
                        if isinstance(v, str) and '"match"' in v and "[" in v:
                            raw_text = v.strip()
                            print(f"    🆘 从 {field} 挖到 JSON: {len(v)}字")
                            break
                
                # 优先级 4: 其他非 thinking 字段中找带 "match" 的
                if not raw_text:
                ...
                if not raw_text:
                    ...
                    if longest and len(longest) > 20:
                        raw_text = longest
                        print(f"    🆘 优先级6: 取最长非thinking字段")
        
        # 优先级 7: OpenAI Assistants v2 风格
        ...
        # 优先级 8: 嵌套 data 字段(如 {"data": {"choices": [...]}})
        if not raw_text and isinstance(data.get("data"), dict):
            nested = data["data"]
            if nested.get("choices"):
                try:
                    raw_text = nested["choices"][0]["message"]["content"] or ""
                    if raw_text:
                        print(f"    🆘 从 data.choices 挖到内容")
                except:
                    pass
        
        # 优先级 9: 终极兜底 — 全字符串 dump 扫描
        if not raw_text:
            full_str = json.dumps(data, ensure_ascii=False)
            m_match = re.search(r'\[\s*\{\s*\\?"match\\?"', full_str)
            if m_match:
                raw_text = _extract_json_from_dump(full_str, m_match.start())
                if raw_text:
                    print(f"    🆘 终极兜底: 从response dump提取JSON")
...
def _extract_json_from_dump(full_str: str, start_pos: int) -> str:
“”“从 JSON dump 字符串里提取一个完整的 […] 数组,支持转义”””
try:
depth = 0
end_pos = start_pos
...
```
v18.1 加固:
- top3 字段兼容 score/比分/scoreline
- ai_confidence 安全整数转换(处理 '85%' 等)
- detected_traps 安全列表转换
- 比分字符串归一化(全角/空格)
- is_valid 判断用 is None,避免 0-0 被当成假
- 空 top3 或全无效 score → 不生成伪造的 "1-1"
- 单 item 崩不影响其他 item(逐 item 回退)
- 宽松 JSON 修复(单引号、尾逗号、None→null)
- Markdown 清理加全(```/`/''')
- 嵌套结构兜底(如 {"predictions": [...]})
"""
...
# 兜底 2: 嵌套结构(如 {"predictions": [...]})
if not json_str:
    for wrapper in ["predictions", "results", "data", "matches", "output"]:
        m = re.search(rf'"{wrapper}"\s*:\s*(\[.+?\])', clean, re.DOTALL)
        if m:
            json_str = m.group(1)
            print(f"    🔍 嵌套结构 {wrapper} 提取JSON: {len(json_str)}字")
            break
...
# ---- 尝试多种方式解析 ----
arr = _robust_json_parse(json_str)
...
for idx, item in enumerate(arr):
    try:
        ...
        # match id 必须存在且能转 int
        mid_raw = item.get("match") or item.get("index") or item.get("id")
        if mid_raw is None:
            continue
        try:
            # 支持字符串 "1" 或数字 1
            if isinstance(mid_raw, str):
                ...
                if mid_raw in cn_map:
                    mid = cn_map[mid_raw]
                else:
                    mid = int(re.search(r"\d+", mid_raw).group())
            else:
                mid = int(mid_raw)
        except:
            continue
        ...
        # 超出范围忽略
        if mid < 1 or mid > num_matches + 5:
            continue
        
        # ---- 提取 top3(兼容 score/比分/scoreline 多字段名)----
        top3_raw = item.get("top3", []) or item.get("top_scores", []) or item.get("candidates", [])
        top3_clean = []
        if isinstance(top3_raw, list):
            for t in top3_raw[:5]:
                try:
                    if isinstance(t, dict):
                        sc_v = t.get("score") or t.get("比分") or t.get("scoreline") or ""
                        sc_norm = _normalize_score_str(sc_v)
                        if not sc_norm:
                            continue
                        if _parse_score(sc_norm)[0] is None:
                            continue
                        prob_v = t.get("prob", t.get("probability", 0))
                        top3_clean.append({
                            "score": sc_norm,
                            "prob": _safe_int(prob_v, 0),
                        })
                    elif isinstance(t, str):
                        sc_norm = _normalize_score_str(t)
                        if sc_norm and _parse_score(sc_norm)[0] is not None:
                            top3_clean.append({"score": sc_norm, "prob": 0})
                except:
                    continue
        ...
        for cand in score_candidates:
            sc_norm = _normalize_score_str(cand)
            if sc_norm and _parse_score(sc_norm)[0] is not None:
                top1_score = sc_norm
                break
        if not top1_score and top3_clean:
            top1_score = top3_clean[0]["score"]
        
        # 如果连 top1 都没有,这个 AI 对此场弃权
        if not top1_score:
            continue
        
        # ---- 构建结果(字段类型安全)----
        reason_raw = item.get("reason", item.get("analysis", item.get("explanation", "")))
        reason = str(reason_raw) if reason_raw is not None else ""
        # 空字符串或"None" → 空
        if reason.strip().lower() in ("none", "null", ""):
            reason = ""
        
        final_dir_raw = str(item.get("final_direction", "")).strip().lower()
        # 中文方向词 → 英文
        dir_cn_map = {"主胜": "home", "客胜": "away", "平局": "draw",
                     "home win": "home", "away win": "away", "draw": "draw",
                     "主": "home", "客": "away", "平": "draw"}
        for k_cn, v_en in dir_cn_map.items():
            if k_cn in final_dir_raw:
                final_dir_raw = v_en
                break
        if final_dir_raw not in ("home", "draw", "away"):
            final_dir_raw = ""
        ...
        results[mid] = {
            ...
            "ai_score": top1_score,
            "reason": reason[:1000],
            "ai_confidence": max(0, min(100, _safe_int(item.get("ai_confidence", 60), 60))),
            "is_score_others": bool(item.get("is_score_others", False)),
            "detected_traps": _ensure_list(item.get("detected_traps", [])),
            "final_direction": final_dir_raw,
        }
    except Exception as ex:
        # 单个 item 崩不影响其他
        print(f"    ⚠️ 第{idx}条item解析失败: {str(ex)[:60]}")
        continue

return results
```

def _robust_json_parse(json_str: str):
“””
多策略 JSON 解析:
1. 直接 json.loads
2. 常见修复:单引号→双引号、尾逗号、None→null、True/False 大小写
3. 断肢修复:截到最后一个完整 }
4. 逐 item 正则提取(最后手段)
“””
# 策略 1: 原样解析
...
# 策略 4: 逐 item 正则提取(最后手段)
try:
    items = []
    # 匹配每个 {"match": N, ...} 块
    pattern = r'\{\s*"match"\s*:\s*\d+.*?\n\s*\}'
    for m in re.finditer(pattern, fixed, re.DOTALL):
        block = m.group(0)
        # 尝试清理后解析
        try:
            ...
            # 再次用宽松修复
            b2 = re.sub(r",\s*([}\]])", r"\1", block)
            b2 = re.sub(r"\bNone\b", "null", b2)
            try:
                item = json.loads(b2)
                items.append(item)
            except:
...
def *save_debug_dump(ai_name, data, tag):
try:
os.makedirs(“data/debug”, exist_ok=True)
dump_file = f”data/debug/{ai_name}*{tag}_{int(time.time())}.json”
