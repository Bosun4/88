...
def _cross_anchor_questions(match_obj: Dict[str, Any]) -> List[str]:
    ...
    if _is_intl:
        qs.append("【零封税·强制】国际赛/世界杯禁止默认开 X-0（对手0球）。除非有显式硬证据（对手主力锋线伤停/三军尽出对鱼腩/对手进攻瘫痪），否则必须假定对手至少进1球：0-3 上修为 1-3/1-4，2-0 上修为 2-1/3-1，3-0 上修为 3-1。数据支撑：国际赛零封预测 73% 失败。")
        qs.append("【进球带上修·同方向】国际赛/世界杯方向判定不变，但同方向比分形状强制上修一档：强队预测净胜球带优先给更高进球的形状（1-0→2-0/2-1，2-0→2-1/3-1，0-3→1-3/1-4）。数据支撑：方向对但比分错的场次低估:高估=5:1，平均少约半球。")
