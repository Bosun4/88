...
def _handicap_anchor_facts(match_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (0 < a4 <= 5.3) or (0 < a5 <= 8.0):
        ...
        tpl["big_goal_injection"] = {
            ...
            "reason": "4球/5球赔率被深度压缩，强制将对称大球带并入must_audit_scores；AI必须主客对称审计，不得机械压回2-1/1-1。",
        }
...
def _cross_anchor_questions(match_obj: Dict[str, Any]) -> List[str]:
    ...
    if 0 < a4 <= 5.3:
        qs.append("⚠4球赔率<=5.3被深度压缩：市场强烈预期4+大球，必须解除小球与1-1/2-1常规锈定，并在top3/候选中主客对称审计大球带：3-1/1-3/2-2/3-2/2-3/4-1/1-4/4-2。若最终仍选≤三球比分，top3必须至少包含1个4+大球比分并说明为何不主推。")
    if mode_g in (0, 1, 2):
...
def build_phase1_prompt(evidence_batch: List[Dict[str, Any]], ai_name: str) -> str:
    ...
    p.append("1. 进攻高波或高进球异动压缩（如德甲、荷甲、美职、挪超、解放者杯等，或当前 4球/5球赔率显示被深度压低，例如4球赔率<=5.3）：防守往往让位于进攻，不得机械保守。不要机械拘泥于 2-1、1-1 等常规最低赔率。若双方战术开放或市场总进球异动支持（例如 4球赔率极低），必须敢于将大球带（3-1、1-3、2-2、3-2、2-3、4-1、1-4、4-2 等主客对称比分）作为主推首选，不要仅仅把它们当做风险尾部藏起来！【对称强制】考虑任一主队大胜比分时，必须同时列出客队镜像比分（3-1↔1-3、4-1↔1-4、3-2↔2-3）。")
