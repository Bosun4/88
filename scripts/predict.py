...
【v20.6 Gemini 终审最高博弈约束：逆向审计、反诱盘、背离探测】
...
4. 输出 predicted_score 前必须完成相邻比分审计：
...
5. 若相邻比分赔率更低或接近但最终不选，必须在 rejected_cases 或 reason 中解释；解释不足时 recommendation.tier 最高 B。
6. 必须读取 sharp_money_facts_v203。若 had_change 缺失，只能说 RLM 不可确认，不能声称“聪明钱已确认”。
7. 若 no_web_tool_available，不能把抓包里的 injury/news/points 包装成实时联网来源； these只能算 packet_context，最高 reliability=medium。
8. 推荐等级必须拆分：direction_edge、score_cluster_strength、goal_band_strength、btts_alignment、sharp_alignment、web_source_quality、market_conflict_penalty。
9. 你必须输出 bet_action：main/small/hedge/observe/no_bet；本地不会替你改足球方向、比分或原始 confidence，只会展示风险提示。
10. 你必须输出 why_this_can_fail 与 minimum_evidence_needed；若关键证据缺失且无法覆盖失败路径，应主动 no_bet/observe，而不是硬给 A/B 推荐。
""".strip()
