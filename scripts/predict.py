...
p += “7. 最终只给1个精确比分，分析必须量化说明为什么这个比分优于其他3个AI\n\n”
p += “【输出】严格纯JSON，禁止任何其他文字：\n”
...
if isinstance(vote, dict) and vote.get(“win”, 0) and float(vote.get(“win”, 0)) > 55 and float(match_obj.get(“sp_home”, 0) or 0) > 1.8:
...
if ai_scores:
ai_home_pct = sum(1 for sc in ai_scores if sc.split(”-”)[0] > sc.split(”-”)[1]) / len(ai_scores) * 100
ai_draw_pct = sum(1 for sc in ai_scores if sc.split(”-”)[0] == sc.split(”-”)[1]) / len(ai_scores) * 100
else:
ai_home_pct = 33
ai_draw_pct = 33
market_home_prob = top_crs[0].get(“prob”, 33) if top_crs else 33
market_draw_prob = top_crs[1].get(“prob”, 33) if len(top_crs) > 1 else 33
fused_hp = (sys_hp * weights[“stat”] + ai_home_pct * weights[“ai”] + market_home_prob * weights[“market”]) / total_w
fused_dp = (sys_dp * weights[“stat”] + ai_draw_pct * weights[“ai”] + market_draw_prob * weights[“market”]) / total_w
fused_ap = max(5, 100 - fused_hp - fused_dp)
for sc in candidates:
if sc == market_top:
candidates[sc][“weight”] += 0.35
if sc in ai_scores and ai_scores.count(sc) >= 2:
candidates[sc][“weight”] += 0.40
if model_cons >= 9 and sc == sys_score:
...
print(f”  11 Stats + 4 AI | Auto-fallback enabled”)
print(f”{’=’*60}”)
res = []
...
print(f”     H{sp[‘home_win_pct’]:.1f}% D{sp[‘draw_pct’]:.1f}% A{sp[‘away_win_pct’]:.1f}% con:{sp.get(‘model_consensus’, 0)}/{sp.get(‘total_models’, 11)}”)
