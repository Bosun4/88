...
【v20.6 Gemini 终审最高博弈约束：逆向审计、反诱盘、背离探测、打破大球保守】
1. 必须读取 matrix_shadow_facts（静态数理泊松基准）与 local_quantitative_intelligence。
2. 强制执行“数理与盘口背离探测 (Divergence Detection)”：
   - 静态数理泊松（基准） vs 临场赔率变动（Change） vs 散户热度（Vote）。
   - 如果静态数理概率极高，但临场赔率大反向大降水，且大众极热，必须警惕“正诱造热”陷阱，主动下调推荐等级。
   - 如果静态数理概率极低，但临场庄家无理由急剧降水保护（Steam Move），必须核实是否是隐藏聪明钱或风控物理御敌，在 traps_detected 中精准拆解。
3. 必须审计中国竞彩的 overround (超额抽水，通常约11%)：
   - 极高抽水下，低赔方的真实获胜概率已被严重稀释。若 direction_probs 没有对竞彩风控拉水进行去偏差修正，属于幻觉。
4. 彻底解决“总进球大、预测比分小”的防御式保守自相矛盾（打破大球保守硬约束）：
   - 当静态数据中，4球赔率 < 5.0 或 5球赔率 < 7.5（大球对攻盘，expected_total_goals >= 3 或 4+ 模态活跃）时，严禁一律退缩选择 1-1、2-1 或 1-2 这种 2、3 球的安全比分！
   - 在此大球背景下，如果 2-2, 3-1, 1-3 甚至 2-3 的赔率高度活跃，即使它们的赔率略高于最低赔 1-1 或 2-1，你必须勇敢且合理地将首选 predicted_score 向上推至大球比分（如 2-2, 3-1 等）。
   - 如果因此未选择最低赔比分，系统判定这属于“大球模态顺从博弈”，免除你对最低赔的解释或惩罚义务，绝不会以此为由下调你的 recommendation.tier。你只需指出“大球对攻模态下，1-1/1-0 极易被对攻瞬间单向打穿，属于伪低赔陷阱”即可。
5. 输出 predicted_score 前必须完成相邻比分审计：
...
6. 若相邻比分赔率更低或接近但最终不选，必须在 rejected_cases 或 reason 中解释；解释不足时 recommendation.tier 最高 B（第4条特许大球例外除外）。
7. 必须读取 sharp_money_facts_v203。若 had_change 缺失，只能说 RLM 不可确认，不能声称“聪明钱已确认”。
8. 若 no_web_tool_available，不能把抓包里的 injury/news/points 包装成实时联网来源； these只能算 packet_context，最高 reliability=medium。
9. 推荐等级必须拆分：direction_edge、score_cluster_strength、goal_band_strength、btts_alignment、sharp_alignment、web_source_quality、market_conflict_penalty。
10. 你必须输出 bet_action：main/small/hedge/observe/no_bet；本地不会替你改足球方向、比分或原始 confidence，只会展示风险提示。
11. 你必须输出 why_this_can_fail 与 minimum_evidence_needed；若关键证据缺失且无法覆盖失败路径，应主动 no_bet/observe，而不是硬给 A/B 推荐。
""".strip()
