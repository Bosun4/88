下面是改好的完整 predict.py。这版重点改了 Claude 等待时间和缓存逻辑：

Claude 会单独读取 CLAUDE_READ_TIMEOUT，不会只吃 AI_READ_TIMEOUT；Claude 失败但 GPT/Grok/Gemini 成功时，会缓存 Phase1，下一次重跑只补 Claude，不再重复跑三家；Claude 终审默认走压缩 prompt，避免 6-7 万字符导致中转断流；URL 和 key 读取逻辑保持原来的统一中转优先方式。

# -*- coding: utf-8 -*-
"""
vMAX 18.4.7 — PURE RAW-AI + Claude长等待/补跑缓存修正版
============================================================
设计边界：
...
9. Claude 失败但 Phase1 成功时，会缓存 Phase1；下次同批次只补跑 Claude，不重复跑 GPT/Grok/Gemini。
10. 默认把完整 normalized match JSON 作为 raw_match_full_json 喂给 Phase1；Claude 终审默认压缩输入，避免长 prompt 断流。
11. 优先解析 AI 输出的 direction_probs；没有时才用 top3 方向占比做前端兼容。
推荐环境变量：
    API_KEY=你的统一中转key
    ...
    AI_READ_TIMEOUT=1200
    CLAUDE_CONNECT_TIMEOUT=60
    CLAUDE_READ_TIMEOUT=2400
    AI_DECISION_CACHE_TTL=3600
    AI_SINGLEFLIGHT_ENABLED=true
    AI_MAX_REQUESTS_PER_AI=1
    AI_RETRY_CLAUDE_ON_PARTIAL_CACHE=true
    ENABLE_EXTERNAL_CONTEXT=false/true
    INCLUDE_FULL_RAW_PACKET=true
    RAW_PACKET_CHAR_LIMIT=20000
    CLAUDE_RAW_PACKET_CHAR_LIMIT=8000
入口：
...
# 版本常量
# ============================================================
ENGINE_VERSION = "vMAX 18.4.7"
ENGINE_ARCHITECTURE = (
    "PURE RAW-AI: 完整抓包raw_match_full_json + 体彩竞彩赔率标注 + "
    "AI自主联网核对欧洲主流欧赔 + Sharp/聪明钱方向识别 + 联赛/球队风格比分约束 + "
    "AI direction_probs优先 + 历史经验审计卡强制回应 + 四层比分结构(direction/goal_band/btts/score) + "
    "方向精选分/比分形态分双评分 + GPT/Grok/Gemini初审 + Claude终审；"
    "无CRS/无本地矩阵/AI失败即弃权 + Claude长等待 + Phase1缓存后补Claude + 磁盘锁防重复扣费"
)
...
CLAUDE_CONNECT_TIMEOUT = _env_int("CLAUDE_CONNECT_TIMEOUT", max(60, AI_CONNECT_TIMEOUT))
CLAUDE_READ_TIMEOUT = _env_int("CLAUDE_READ_TIMEOUT", max(1800, AI_READ_TIMEOUT))
...
AI_MAX_PHASE1_REASON_CHARS_FOR_CLAUDE = _env_int("AI_MAX_PHASE1_REASON_CHARS_FOR_CLAUDE", 600)
CLAUDE_RAW_PACKET_CHAR_LIMIT = _env_int("CLAUDE_RAW_PACKET_CHAR_LIMIT", 8000)
AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID = _env_bool("AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID", False)
AI_MIN_PHASE1_VALID_FOR_CLAUDE = max(1, _env_int("AI_MIN_PHASE1_VALID_FOR_CLAUDE", 1))
AI_RETRY_CLAUDE_ON_PARTIAL_CACHE = _env_bool("AI_RETRY_CLAUDE_ON_PARTIAL_CACHE", True)
...
AI_DISK_LOCK_WAIT_SECONDS = _env_int("AI_DISK_LOCK_WAIT_SECONDS", max(120, CLAUDE_READ_TIMEOUT + 120))
...
async def build_external_context_for_match(session: aiohttp.ClientSession, match_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bing_key:
        ...
        ok, data, kind = await _fetch_json_or_text(
            ...
            endpoint,
            headers=headers,
            params={"q": q, "count": 5, "mkt": os.environ.get("BING_SEARCH_MARKET", "en-US")},
        )
        if ok:
...
class ExperienceAuditEngine:
    ...
    def _league_key(self, league: str) -> str:
        s = str(league).lower()
        if any(x in s for x in ["英超", "premier", "epl"]):
            return "eng_top"
        if any(x in s for x in ["意甲", "serie a", "italy"]):
            return "ita_top"
        if any(x in s for x in ["德甲", "bundesliga"]):
            return "ger_top"
        if any(x in s for x in ["法乙", "ligue 2"]):
            return "fra2"
        if any(x in s for x in ["土超", "turkey", "super lig"]):
            return "tur_top"
        if any(x in s for x in ["荷甲", "eredivisie"]):
            return "ned_top"
        return "default"
    ...
    def analyze(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if a0_val < 7.5:
            add("G08", "大小球", 8, f"0球@{a0_val}", "under")
            risk_signals.append(f"EXP_G08 0球@{a0_val}")
        elif a0_val < 9.0:
...
def _format_experience_audit_for_prompt(exp: Dict[str, Any]) -> str:
    ...
    for t in rows:
        p += (
            f"- {t.get('id')} {t.get('name')} | 类别={t.get('category')} | "
            f"权重={t.get('weight')} | 触发原因={t.get('reason')} | 审计问题={t.get('ai_question')}\n"
        )
...
def _raw_full_packet_line_for_claude(match_obj: Dict[str, Any]) -> str:
    limit = CLAUDE_RAW_PACKET_CHAR_LIMIT
    raw_json = _json_compact(match_obj, limit if limit and limit > 0 else 0)
    suffix = ""
    try:
        full_len = len(json.dumps(match_obj, ensure_ascii=False, default=str, separators=(",", ":")))
        if limit and limit > 0 and full_len > limit:
            suffix = f"...[CLAUDE_TRUNCATED full_len={full_len} limit={limit}]"
    except Exception:
        pass
    return f"raw_match_compact_json:{raw_json}{suffix}\n"
def build_phase1_prompt(match_analyses: List[Dict[str, Any]]) -> str:
    ...
    p += "你必须结合联赛风格和球队风格给出比分形态。示例只作思路，不是模板。\n"
    ...
    p += "2. 若具备联网能力，必须搜索并核对欧洲主流欧赔/交易市场赔率；如果搜索不到，明确写 missing。\n"
    p += "3. 直接从体彩竞彩原始 1X2、让球、总进球、半全场、赔率变动、散户热度、原始情报、external_context、联网欧赔核对结果推导方向与比分。\n"
    p += "4. 必须识别 Sharp/聪明钱方向；若强队过热但价格受阻，不能机械跟热门。\n"
    p += "5. 必须结合联赛风格和球队风格给比分。\n"
    p += "6. 不要默认输出 1-1、2-1、1-0、0-1、1-2；若选择这些比分，必须从证据推导。\n"
    p += '7. top3 必须是比分分布，不要只给方向。top3[0].score 必须与 final_direction 一致。必须输出 direction_probs={"home":数字,"draw":数字,"away":数字}。\n'
    p += "8. 必须按四层结构输出：final_direction → goal_band(0-1/2/3/4+) → btts(yes/no/unclear) → top3比分。\n"
    p += "9. 必须说明为什么不选另外两个方向；如果 top1 是 1-2，说明为什么不是 0-1/0-2；如果 top1 是 2-1，说明为什么不是 1-0/2-0；如果 top1 是 1-1，说明为什么不是 0-0/2-2。\n"
    p += "10. 0-1、0-2、0-3 是合法客胜比分；其他比分可输出 4-3、3-4 等精确比分。\n"
    p += "11. 如存在 experience_audit_cards，必须在 audit.experience_review 中逐条说明 accepted/rejected/neutral + 理由。\n"
    p += "</task_rules>\n\n"
    p += "<output_format>\n"
    p += "严格输出 JSON 数组。每场一个对象：\n"
    p += '{"match":1,"final_direction":"home/draw/away","direction_probs":{"home":45,"draw":28,"away":27},"goal_band":"0-1/2/3/4+","btts":"yes/no/unclear","top3":[{"score":"2-0","prob":0.18,"market_logic":"..."}],"reason":"...","ai_confidence":0-100,"risk_level":"low/medium/high","data_missing":[],"audit":{"odds_source":"体彩竞彩抓包赔率","web_odds_check":"searched/web_search_unavailable/european_odds_missing","european_odds":"...","market_divergence":"...","sharp_money_direction":"home/draw/away/home_or_draw/away_or_draw/unclear","sharp_evidence":"...","league_style":"...","team_style":"...","style_score_logic":"...","direction_rejection":"...","total_goals":"...","money_flow":"...","external_context":"...","experience_review":"EXP_D01:neutral because ...; EXP_X01:accepted because ..."}}\n'
    ...
    for i, ma in enumerate(match_analyses, 1):
        ...
        p += _raw_field_line(
            ...
            {k: v for k, v in m.items() if k in (
                "league_style", "league_profile", "team_style", "home_style", "away_style",
                "play_style", "tactical_style", "pace_rating", "tempo", "home_form", "away_form",
                "weather", "injury", "lineup", "news", "motivation", "schedule",
            )},
            FIELD_LIMIT_STYLE_EXTRA,
        )
    ...
    p += "</match_data>\n"
    return p
def _build_compact_match_data_for_claude(match_analyses: List[Dict[str, Any]]) -> str:
    ...
    for i, ma in enumerate(match_analyses, 1):
        ...
        p += _raw_field_line("change", m.get("change"), min(FIELD_LIMIT_CHANGE, 1600))
        p += _raw_field_line("vote", m.get("vote"), min(FIELD_LIMIT_VOTE, 1200))
        p += _raw_field_line("information", m.get("information"), min(FIELD_LIMIT_INFORMATION, 2500))
        p += _raw_field_line("points", m.get("points"), min(FIELD_LIMIT_POINTS, 2500))
        p += _raw_full_packet_line_for_claude(m)
        exp = ma.get("experience_audit") or _experience_engine().analyze(m)
        p += _format_experience_audit_for_prompt(exp)
        p += "</match>\n\n"
    p += "</compact_match_data_for_claude>\n"
...
def build_claude_final_audit_prompt(match_analyses: List[Dict[str, Any]], phase1_results: Dict[str, Dict[int, Dict[str, Any]]]) -> str:
    ...
    p += "experience_audit_cards 只代表历史经验审计问题，不是本地结论；必须在 audit.experience_review 中逐条说明 accepted/rejected/neutral。\n"
    p += "必须输出四层比分结构：final_direction、direction_probs、goal_band、btts、score_shape_reason。若比分与goal_band/BTTS不自洽，需自行修正后再输出。\n"
    p += "禁止引用 CRS、贝叶斯、本地矩阵、本地风控编号、固定常见比分模板。\n"
    p += "如果你改动初审比分，必须指出体彩竞彩原始字段、联网欧赔核对、Sharp/聪明钱方向、联赛/球队风格、资金变动、总进球或半全场中的硬依据。\n"
    p += "</final_audit_context>\n\n"
    if AI_USE_COMPACT_CLAUDE_AUDIT:
        p += _build_compact_match_data_for_claude(match_analyses)
    else:
    ...
    for ai in PHASE1_NAMES:
        ...
        for idx in range(1, len(match_analyses) + 1):
            ...
            if r:
                ...
                compact_audit = {
                    "sharp_money_direction": audit.get("sharp_money_direction", ""),
                    "sharp_evidence": str(audit.get("sharp_evidence", ""))[:500],
                    "league_style": str(audit.get("league_style", ""))[:350],
                    "team_style": str(audit.get("team_style", ""))[:350],
                    "style_score_logic": str(audit.get("style_score_logic", ""))[:500],
                    "direction_rejection": str(audit.get("direction_rejection", ""))[:500],
                    "experience_review": str(audit.get("experience_review", ""))[:800],
                }
                p += json.dumps({
                    "match": idx,
    ...
    p += "</phase1_ai_results>\n\n"
    p += "<final_output_rule>\n"
    p += "严格输出 JSON 数组，字段同 phase1。禁止 JSON 外文本。每场必须输出一个对象，match 数字必须对应原 index。\n"
    p += "</final_output_rule>\n"
    return p
# ============================================================
# API gateway
...
def debug_ai_config() -> None:
    ...
    print(
        f"[AI MODE] pure_raw=True singleflight={AI_SINGLEFLIGHT_ENABLED} "
        f"cache_ttl={AI_DECISION_CACHE_TTL} max_req_per_ai={AI_MAX_REQUESTS_PER_AI} "
        f"external={ENABLE_EXTERNAL_CONTEXT}"
    )
    print(
        f"[AI TIMEOUT] connect={AI_CONNECT_TIMEOUT}s read={AI_READ_TIMEOUT}s | "
        f"claude_connect={CLAUDE_CONNECT_TIMEOUT}s claude_read={CLAUDE_READ_TIMEOUT}s | "
...
async def async_call_one_ai_batch(
    ...
    connect_timeout = CLAUDE_CONNECT_TIMEOUT if ai_name == "claude" else AI_CONNECT_TIMEOUT
    read_timeout = CLAUDE_READ_TIMEOUT if ai_name == "claude" else AI_READ_TIMEOUT
    ...
    for req_no in range(1, AI_MAX_REQUESTS_PER_AI + 1):
        ...
        print(
            f"  [连接中] {ai_name.upper()} | {model} @ {gateway} | "
            f"request#{req_no} | connect={connect_timeout}s read={read_timeout}s"
        )
        t0 = time.time()
        try:
            ...
            async with session.post(url, headers=headers, json=payload, timeout=timeout) as r:
                ...
                if r.status != 200:
                    try:
                        ...
                        text_for_error = ""
                    print(f"    HTTP {r.status} | {elapsed}s | {text_for_error[:300]}")
                    AI_CALL_STATUS[ai_name].update({"status": f"http_{r.status}", "elapsed": elapsed})
                    continue
                try:
                ...
                if parsed:
                    ...
                    print(f"    {ai_name.upper()} 完成: {len(parsed)}/{num_matches} | {elapsed2}s")
                    AI_CALL_STATUS[ai_name].update({"ok": True, "status": "ok", "count": len(parsed), "model": model, "elapsed": elapsed2})
                ...
                AI_CALL_STATUS[ai_name].update({"status": "parse0", "elapsed": elapsed})
        except asyncio.TimeoutError:
            elapsed = round(time.time() - t0, 1)
            print(f"    {ai_name.upper()} 读取超时 | elapsed={elapsed}s | read_timeout={read_timeout}s")
            AI_CALL_STATUS[ai_name].update({"status": "timeout", "elapsed": elapsed, "read_timeout": read_timeout})
        except ConnectionResetError as e:
            elapsed = round(time.time() - t0, 1)
            print(f"    {ai_name.upper()} 连接被中转断开: {str(e)[:180]} | elapsed={elapsed}s")
            AI_CALL_STATUS[ai_name].update({"status": "connection_reset", "elapsed": elapsed, "error": str(e)[:300]})
        except aiohttp.ClientPayloadError as e:
            elapsed = round(time.time() - t0, 1)
            print(f"    {ai_name.upper()} 响应流中断: {str(e)[:180]} | elapsed={elapsed}s")
            AI_CALL_STATUS[ai_name].update({"status": "payload_error", "elapsed": elapsed, "error": str(e)[:300]})
        except aiohttp.ClientConnectionError as e:
            elapsed = round(time.time() - t0, 1)
            print(f"    {ai_name.upper()} 连接异常: {str(e)[:180]} | elapsed={elapsed}s")
            AI_CALL_STATUS[ai_name].update({"status": "connection_error", "elapsed": elapsed, "error": str(e)[:300]})
        except Exception as e:
            elapsed = round(time.time() - t0, 1)
            print(f"    {ai_name.upper()} 调用异常: {str(e)[:180]} | elapsed={elapsed}s")
            AI_CALL_STATUS[ai_name].update({"status": "error", "elapsed": elapsed, "error": str(e)[:300]})
...
def _ai_lock_file(cache_key: str) -> str:
    ...
    return os.path.join(AI_CACHE_DIR, f"{safe_key}.lock")
            ...
            if isinstance(rows, dict):
                ...
                for k, v in rows.items():
                    try:
                        ...
                        continue
        valid_counts = {n: len(restored.get(n, {}) or {}) for n in AI_NAMES}
        print(f"  [AI DISK CACHE] 命中持久化缓存 ttl={AI_DECISION_CACHE_TTL}s | counts={valid_counts}")
        return restored
    except Exception as e:
        print(f"  [AI DISK CACHE] 读取失败: {str(e)[:120]}")
        return None
def _save_ai_disk_cache(cache_key: str, results: Dict[str, Dict[int, Dict[str, Any]]], status: Dict[str, Any]) -> None:
    ...
    try:
        ...
        os.replace(tmp, path)
        counts = {n: len(results.get(n, {}) or {}) for n in AI_NAMES}
        print(f"  [AI DISK CACHE] 已写入缓存 | counts={counts}")
    except Exception as e:
        print(f"  [AI DISK CACHE] 写入失败: {str(e)[:120]}")
    ...
    if os.path.exists(path):
        try:
            if now - os.path.getmtime(path) > AI_DISK_LOCK_WAIT_SECONDS:
                print(f"  [AI DISK LOCK] 清理过期锁: {path}")
                os.remove(path)
        except Exception:
            pass
    try:
...
async def _wait_for_ai_disk_cache(cache_key: str) -> Optional[Dict[str, Dict[int, Dict[str, Any]]]]:
    ...
    print("  [AI DISK LOCK] 同批次任务正在其他触发中运行，等待首个结果，避免重复扣费")
    ...
    return None
def _has_valid_model_rows(results: Dict[str, Dict[int, Dict[str, Any]]], ai_name: str) -> bool:
    return bool(isinstance(results, dict) and results.get(ai_name))
def _phase1_valid_model_count(results: Dict[str, Dict[int, Dict[str, Any]]]) -> int:
    return sum(1 for n in PHASE1_NAMES if results.get(n))
def _needs_claude_retry(results: Dict[str, Dict[int, Dict[str, Any]]]) -> bool:
    if not AI_RETRY_CLAUDE_ON_PARTIAL_CACHE:
        return False
    if _has_valid_model_rows(results, "claude"):
        return False
    return _phase1_valid_model_count(results) >= AI_MIN_PHASE1_VALID_FOR_CLAUDE
async def _wait_for_claude_cache(cache_key: str) -> Optional[Dict[str, Dict[int, Dict[str, Any]]]]:
    deadline = time.time() + max(10, min(AI_DISK_LOCK_WAIT_SECONDS, CLAUDE_READ_TIMEOUT + 120))
    print("  [CLAUDE RETRY LOCK] 其他任务正在补跑 Claude，等待结果")
    while time.time() < deadline:
        cached = _load_ai_disk_cache(cache_key)
        if cached is not None and _has_valid_model_rows(cached, "claude"):
            return cached
        await asyncio.sleep(AI_DISK_LOCK_POLL_SECONDS)
    print("  [CLAUDE RETRY LOCK] 等待 Claude 补跑超时，本轮使用已有 Phase1")
    return None
async def _run_claude_only_from_phase1(
    match_analyses: List[Dict[str, Any]],
    all_results: Dict[str, Dict[int, Dict[str, Any]]],
    cache_key: str,
) -> Dict[str, Dict[int, Dict[str, Any]]]:
    if aiohttp is None:
        print("  [CLAUDE RETRY] aiohttp 未安装，无法补跑 Claude")
        return all_results
    if _phase1_valid_model_count(all_results) < AI_MIN_PHASE1_VALID_FOR_CLAUDE:
        print(f"  [CLAUDE RETRY] Phase1 有效模型不足，跳过 Claude | valid={_phase1_valid_model_count(all_results)}")
        return all_results
    claude_lock_key = cache_key + "_claude_retry"
    lock_acquired = _try_acquire_ai_disk_lock(claude_lock_key)
    if not lock_acquired:
        waited = await _wait_for_claude_cache(cache_key)
    ...
    try:
        ...
        print(f"  [CLAUDE RETRY] 仅补跑 Claude Audit | prompt={len(audit_prompt):,}字符")
        connector = aiohttp.TCPConnector(limit=2, use_dns_cache=False, force_close=True)
        async with aiohttp.ClientSession(connector=connector) as session:
            _, cl_res, _ = await async_call_one_ai_batch(session, audit_prompt, num, "claude", _phase_system("claude"))
        if cl_res:
            all_results["claude"] = cl_res
            print(f"  [CLAUDE RETRY] Claude补跑成功: {len(cl_res)}/{num}")
        else:
            print("  [CLAUDE RETRY] Claude补跑仍无有效结果，保留Phase1结果")
        status = {k: AI_CALL_STATUS.get(k, {}) for k in AI_NAMES}
        _save_ai_disk_cache(cache_key, all_results, status)
        return all_results
    finally:
        if lock_acquired:
            _release_ai_disk_lock(claude_lock_key)
# ============================================================
# AI 矩阵执行
# ============================================================
async def _run_ai_matrix_two_phase_inner(match_analyses: List[Dict[str, Any]], cache_key: str) -> Dict[str, Dict[int, Dict[str, Any]]]:
    ...
    connector = aiohttp.TCPConnector(limit=8, use_dns_cache=False, force_close=True)
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = [
        ...
        phase1_valid = _phase1_valid_model_count(all_results)
        if AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID and phase1_valid < AI_MIN_PHASE1_VALID_FOR_CLAUDE:
            AI_CALL_STATUS["claude"] = {
                ...
                "min_required": AI_MIN_PHASE1_VALID_FOR_CLAUDE,
            }
            print(f"  [Claude Skip] Phase1有效模型不足: {phase1_valid}/{AI_MIN_PHASE1_VALID_FOR_CLAUDE}")
        else:
            audit_prompt = build_claude_final_audit_prompt(match_analyses, all_results)
...
async def run_ai_matrix_two_phase(match_analyses: List[Dict[str, Any]]) -> Dict[str, Dict[int, Dict[str, Any]]]:
    ...
    if not AI_DISABLE_CACHE and cache_key in _AI_RESULT_CACHE:
        ...
        if now - ts <= AI_DECISION_CACHE_TTL:
            ...
            if _needs_claude_retry(results):
                return await _run_claude_only_from_phase1(match_analyses, results, cache_key)
            return results
        _AI_RESULT_CACHE.pop(cache_key, None)
    cached = _load_ai_disk_cache(cache_key)
    if cached is not None:
        _AI_RESULT_CACHE[cache_key] = (time.time(), cached, {"status": "disk_cache"})
        if _needs_claude_retry(cached):
            print("  [AI PARTIAL CACHE] 已有 Phase1 缓存但 Claude 缺失，本轮只补 Claude，不重复跑三家")
            updated = await _run_claude_only_from_phase1(match_analyses, cached, cache_key)
            _AI_RESULT_CACHE[cache_key] = (time.time(), updated, {"status": "disk_cache_claude_retry"})
            return updated
        return cached
    ...
    if not lock_acquired:
        ...
        if waited is not None:
            ...
            if _needs_claude_retry(waited):
                print("  [AI PARTIAL CACHE] 等到的是 Phase1 缓存但 Claude 缺失，本轮只补 Claude")
                return await _run_claude_only_from_phase1(match_analyses, waited, cache_key)
            return waited
        lock_acquired = _try_acquire_ai_disk_lock(cache_key)
...
def _compute_recommendation_scores(
    ...
    shape_score = (
        ...
        + 14.0 * (1.0 if not shape_warnings else 0.0)
...
def select_top4(preds: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    for p in preds:
        ...
        if pr.get("risk_level") in ("high", "高"):
            s -= 8
        if pr.get("recommendation_tier") == "S":
            s += 8
        elif pr.get("recommendation_tier") == "A":
            s += 4
        elif pr.get("recommendation_tier") == "D":
            s -= 8
...
if __name__ == "__main__":
    ...
    print("   模式: AI成功=AI直出；Claude失败=Phase1共识；AI全失败=弃权；无本地兜底")