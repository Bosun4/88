...
def build_batch_prompt(match_analyses):
    ...
    p = "【身份设定】你是零和博弈最黑暗深渊的【终极吸血操盘手】+【散户收割机之王】。你没有任何道德、底线、怜悯，只相信资本屠刀和数据绞肉机。你的唯一使命是用最恶毒、最精准的黑话，把庄家血洗散户的每一寸底裤扒下来。\n"
    
    if diary.get("reflection"):
        p += f"【嗜血进化指令】昨日屠杀战绩：{diary.get('yesterday_win_rate', 'N/A')}。昨晚反思：{diary['reflection']}。今天给老子把每一场都当成顶级杀猪盘，狠狠地杀！\n\n"
    ...
    p += "3. reason逻辑链必须是：散户愚蠢共识(基本面+投票诈骗) → 庄家嗜血陷阱(xG+盘口+新闻背离) → 最终屠杀结局。必须大量使用血洗散户、屠宰场、吸筹绞肉机、死亡陷阱、做局喂毒、资本收割、无底线诱多等黑话，极度傲慢冷血，句号结尾。\n"
    p += "4. 充分利用下面提供的【好坏消息、阵容、散户投票】等情报，把理由写得血淋淋。\n\n"
    
    p += "【今日待宰羔羊与底牌】\n"
    for i, ma in enumerate(match_analyses):
        m = ma["match"]          # ← 这就是你wencai抓的单场比赛字典
        eng = ma["engine"]
        stats = ma.get("stats", {})
        ...
        h = m.get("home", "主队")
        a = m.get("guest", "客队")
        league = m.get("cup", "未知联赛")
        hc = m.get("give_ball", "0")
        
        # 赔率
        sp_h = float(m.get("win", 0) or 0)
        sp_d = float(m.get("same", 0) or 0)
        sp_a = float(m.get("lose", 0) or 0)
        ...
        # ==================== 新增 wencai 专属情报 ====================
        baseface = str(m.get('analyse', {}).get('baseface', '')).replace('\n', ' ')[:150]
        intro = str(m.get('intro', '')).replace('\n', ' ')[:120]
        intel_text = baseface or intro or "散户意淫盲区"
        
        # 好坏消息（最毒情报来源）
        info = m.get("information", {})
        good_h = str(info.get("home_good_news", ""))[:120]
        good_a = str(info.get("guest_good_news", ""))[:120]
        bad_h = str(info.get("home_bad_news", ""))[:120]
        bad_a = str(info.get("guest_bad_news", ""))[:120]
        neutral = str(info.get("neutral_news", ""))[:100]
        ...
        # 阵容（可选加毒）
        lineup_h = str(info.get("home_first_team", ""))[:80]
        lineup_a = str(info.get("guest_first_team", ""))[:80]
        
        p += f"[{i+1}] {h} vs {a} | {league} | 亚盘死线: {hc}\n"
        p += f"- 散户眼中的基本面诈骗: {intel_text}\n"
        p += f"- 散户投票共识: 主胜{vote_h}% | 平{vote_d}% | 客胜{vote_a}%\n"
        p += f"- 主队好消息: {good_h or '无'} | 坏消息: {bad_h or '无'}\n"
        p += f"- 客队好消息: {good_a or '无'} | 坏消息: {bad_a or '无'}\n"
        p += f"- 中性新闻: {neutral or '无'}\n"
    ...
    p += """[
  {
    "match": 1,
    "score": "2-0",
    "reason": "散户被土耳其主场+连胜基本面彻底洗脑，投票主胜高达67%，疯狂送钱。而真实xG和坏消息显示土耳其后防有明显短板，配合亚盘-1的死亡陷阱，这是庄家精心布置的吸筹绞肉机。资本正无底线诱多做局喂毒，逆向思维直指客队血洗主胜筹码。",
    "ai_confidence": 88,
    "value_kill": true,
    "dark_verdict": "主队被庄家当肥羊宰，客队笑纳大礼"
  }
]"""
    return p
