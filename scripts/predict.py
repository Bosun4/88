...
# ====================================================================
# 🛡️ vMAX 17.0 方案B — 终极量化典藏版
#
# 核心变革:
#   1. Power Method (幂法) 消除波胆赔率中的 Favorite-Longshot Bias
#   2. Multi-Agent 特征提取，消除 AI 字符串投票导致的票数稀释
#   3. 贝叶斯质心漂移 (Bayesian Drift) 替代僵硬的 if-else 加分
#   4. Softmax 函数输出连续概率面，对接真实的 EV 与 Kelly 计算
#   5. 🔥 铁血一致性护栏：绝对禁止 "0-1配主胜" 这种逻辑错位
# ====================================================================
try:
    import structlog
...
def calculate_value_bet(prob_pct, odds):
    ...
    kelly = ((b * prob) - q) / b
    return {"ev": round(ev * 100, 2), "kelly": round(max(0.0, kelly * 0.35) * 100, 2), "is_value": ev > 0.05}
...
def find_power_k(odds_list, target_margin=1.0, tol=1e-4, max_iter=50):
    ...
    for _ in range(max_iter):
        ...
        if abs(current - target_margin) < tol: return mid
        elif current > target_margin: low = mid
        else: high = mid
    return (low + high) / 2
...
def crs_implied_probabilities(match_obj):
    ...
    for score, key in CRS_FULL_MAP.items():
        try:
            ...
            if odds > 1.1: raw_odds[score] = odds
        except: pass
    extras = {}
    for key, scores_set in [("crs_win", SCORE_OTHERS_HOME), ("crs_same", SCORE_OTHERS_DRAW), ("crs_lose", SCORE_OTHERS_AWAY)]:
        try:
            odds = float(match_obj.get(key, 0) or 0)
            if odds > 1.1: extras[key] = {"odds": odds, "scores": scores_set}
        except: pass

    if len(raw_odds) < 8: return {}, 0.0, 0.0
    ...
    probs = {}
    for score, odds in raw_odds.items(): probs[score] = ((1 / odds) ** k_opt) * 100
    for key, extra_data in extras.items():
        ...
        if num_scores > 0:
            ...
            for sc in extra_data["scores"]: probs[sc] = probs.get(sc, 0) + per_score
...
def detect_goal_signals(match_obj):
    ...
    for g in range(8):
        try:
            ...
            if actual > 1:
                ratio = STANDARD_GOAL_ODDS.get(g, 50) / actual
                if ratio > 1.2: signals[g] = ratio
        except: pass
    return signals
...
def detect_score_others(match_obj, exp_goals, ai_responses=None):
    ...
    try:
        ...
        if 0 < a7 <= 23.0: score += 2; triggers.append(f"7+球极低{a7:.1f}≤23"); is_extreme_blowout = True
        elif 0 < a7 <= 30.0: score += 1; triggers.append(f"7+球{a7:.1f}≤30")
        elif 0 < a7 <= 18: score += 1; triggers.append(f"7+球{a7:.1f}≤18")
    except: pass
    try:
        a6 = float(match_obj.get("a6", 999) or 999)
        if 0 < a6 <= 13.0: score += 2; triggers.append(f"6球极低{a6:.1f}≤13"); is_extreme_blowout = True
        elif 0 < a6 <= 16.0: score += 1; triggers.append(f"6球{a6:.1f}≤16")
    except: pass
    try:
        a5 = float(match_obj.get("a5", 999) or 999)
        if 0 < a5 <= 8.0: score += 2; triggers.append(f"5球极低{a5:.1f}≤8"); is_extreme_blowout = True
        elif 0 < a5 <= 10.0: score += 1; triggers.append(f"5球{a5:.1f}≤10")
    except: pass

    if exp_goals >= 3.2: score += 1; triggers.append(f"λ={exp_goals:.2f}≥3.2")
    try:
        info_text = ""
        ...
        if h_match and a_match:
            h_avg = float(h_match.group(1)); a_avg = float(a_match.group(1))
            if (h_avg + a_avg) >= 3.5: score += 1; triggers.append(f"场均{h_avg+a_avg:.1f}≥3.5")
    except: pass
    ...
    try:
        ...
        if 0 < crs_win < crs_same * 0.4 and crs_win < 100: score += 1; triggers.append(f"crs_win{crs_win:.0f}暗示主胜其他")
        elif 0 < crs_lose < crs_same * 0.4 and crs_lose < 100: score += 1; triggers.append(f"crs_lose{crs_lose:.0f}暗示负其他")
    except: pass

    league = str(match_obj.get("cup", match_obj.get("league", "")))
    if any(kw in league for kw in ["欧冠", "欧联", "杯", "淘汰", "决赛"]): score += 0.5; triggers.append("杯赛/淘汰赛")

    ai_others_count = sum(1 for r in (ai_responses or {}).values() if isinstance(r, dict) and r.get("is_score_others"))
    if ai_others_count >= 2: score += 1; triggers.append(f"AI{ai_others_count}/4识别胜其他")

    direction = "home"
    try:
        if crs_lose < crs_win and crs_lose < crs_same: direction = "away"
        elif crs_same < crs_win and crs_same < crs_lose: direction = "draw"
    except: pass

    return {"is_others": score >= 2, "is_extreme_blowout": is_extreme_blowout, "trigger_count": score, "direction": direction, "triggers": triggers, "ai_others_count": ai_others_count}
...
class ColdDoorDetector:
    ...
    def detect(match, prediction):
        ...
        if "Sharp" in smart_str or "sharp" in smart_str: strength += 6; signals.append("🔥 Sharp Money确认！")
        if steam.get("steam") and "反向" in str(steam.get("signal", "")): signals.append("❄️ 反向Steam！庄家造热收割"); strength += 5

        try:
            vh, va = int(match.get("vote", {}).get("win", 33)), int(match.get("vote", {}).get("lose", 33))
            max_vote = max(vh, va)
        ...
        if isinstance(info, dict):
            hp, ap = prediction.get("home_win_pct", 50), prediction.get("away_win_pct", 50)
            if len(str(info.get("home_bad_news", ""))) > 80 and hp > 58: signals.append("❄️ 主队坏消息爆炸+散户狂热"); strength += 5
            if len(str(info.get("guest_bad_news", ""))) > 80 and ap > 58: signals.append("❄️ 客队坏消息爆炸+散户狂热"); strength += 5
        ...
        return {"is_cold_door": is_cold, "strength": strength, "level": level, "signals": signals, "sharp_confirmed": "Sharp" in smart_str, "dark_verdict": f"❄️ {level}冷门！{len(signals)}条触发" if is_cold else ""}
...
def build_phase1_prompt(match_analyses):
    p = "作为量化分析师团队，你面临以下赛事的最终裁决。Python量化引擎已为你算好异常数据，请推理庄家意图。\n\n"
    p += "【输出格式】只输出JSON数组。每场包含:\n"
    p += "  match(整数), top3([{score,prob}],...), reason(350字推理庄家意图), \n"
    p += "  is_score_others(true/false), detected_signals([\"冷门高危\"])\n\n"

    for i, ma in enumerate(match_analyses):
        m = ma["match"]
        eng, stats = ma.get("engine", {}), ma.get("stats", {})
        h, a = m.get("home_team", m.get("home", "Home")), m.get("away_team", m.get("guest", "Away"))
        sp_h, sp_d, sp_a = float(m.get("sp_home", m.get("win", 0)) or 0), float(m.get("sp_draw", m.get("same", 0)) or 0), float(m.get("sp_away", m.get("lose", 0)) or 0)
        
        p += f"{'='*50}\n[{i+1}] {h} vs {a} | {m.get('league', m.get('cup', ''))}\n"
        p += f"基础欧赔: {sp_h:.2f}/{sp_d:.2f}/{sp_a:.2f} | 引擎xG: 主{eng.get('bookmaker_implied_home_xg', '?')} vs 客{eng.get('bookmaker_implied_away_xg', '?')}\n"

        smart_sigs = stats.get('smart_signals', [])
        if smart_sigs: p += f"🔥 资金盘异动探针: {', '.join(str(s) for s in smart_sigs[:5])}\n"
        
        vote = m.get("vote", {})
        if vote:
            max_v = max(int(vote.get('win', 33)), int(vote.get('lose', 33)))
            if max_v >= 58: p += f"⚠️ 散户热度异常: {max_v}% (注意诱盘反杀)\n"

        a_list = [f"{g}球" for g in range(8) if float(m.get(f"a{g}", 0) or 0) > 1 and STANDARD_GOAL_ODDS.get(g, 50) / float(m.get(f"a{g}", 0) or 0) > 1.4]
        if a_list: p += f"🚨 极限压低赔率进球数: {', '.join(a_list)}\n"
        
        info = m.get("information", {})
        if isinstance(info, dict):
            for k, label in [("home_injury", "主伤停"), ("guest_bad_news", "客利空")]:
                if info.get(k): p += f"{label}: {str(info[k])[:150]}\n"
        p += "\n"
    return p + f"【请严格按JSON格式输出{len(match_analyses)}场比赛裁决】\n"

# ====================================================================
# AI调用引擎 - Agentic Workflow 
# ====================================================================
...
async def async_call_one_ai_batch(session, prompt, url_env, key_env, models_list, num_matches, ai_name):
    ...
    urls = [primary_url] if ai_name == "gpt" else [primary_url] + [u for u in FALLBACK_URLS if u and u != primary_url][:1]

    CONNECT_TIMEOUT, READ_TIMEOUT = 20, {"claude": 350, "grok": 250, "gpt": 250, "gemini": 250}.get(ai_name, 200)

    AI_PROFILES = {
        "claude": {"sys": "你是红军风控长(诱盘审查官)。进行红蓝对抗：找出隐蔽的爆冷逻辑。若找不出才允许预测正路。只输出JSON数组。", "temp": 0.25},
        "grok": {"sys": "你是情报搜集官。专注球队的战意陷阱、大热必死情绪。不用算数学，专注心理战。只输出JSON数组。", "temp": 0.3},
        "gpt": {"sys": "你是蓝军数据分析师。专注 xG 偏差和进球数压低。严谨执行庄家的真实进球预期。只输出JSON数组。", "temp": 0.15},
        "gemini": {"sys": "你是模式识别引擎。结合红蓝视角对剧本进行综合评估，输出最终比分预测。只输出JSON数组。", "temp": 0.15},
    }
    ...
    for mn in models_list:
        for base_url in urls:
            ...
            url = base_url.rstrip("/") + ("" if is_gem or "chat/completions" in base_url else "/chat/completions")
            ...
            try:
                async with session.post(url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=None, connect=CONNECT_TIMEOUT, sock_connect=CONNECT_TIMEOUT, sock_read=READ_TIMEOUT)) as r:
                    ...
                    try:
                        ...
                        else: raw_text = data.get("choices", [{}])[0].get("message", {}).get("content", "").strip()
                    except: pass
                    ...
                    if json_str:
                        ...
                        if isinstance(arr, list):
                            for item in arr:
                                ...
                                results[mid] = {"ai_score": str(t1).replace(" ", "").strip(), "reason": str(item.get("reason", ""))[:800], "is_score_others": bool(item.get("is_score_others", False))}
...
async def run_ai_matrix_two_phase(match_analyses):
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=10, ttl_dns_cache=300)) as session:
...
# ====================================================================
# 🌟 Merge v17.0 — 贝叶斯漂移 + Softmax + 铁血护栏版
# ====================================================================
def merge_result(engine_result, gpt_r, grok_r, gemini_r, claude_r, stats, match_obj):
    if isinstance(match_obj.get("v2_odds_dict"), dict):
        match_obj = {**match_obj, **match_obj["v2_odds_dict"]}
    ...
    def _is_valid_ai(r):
        ...
        try:
            ...
            int(p[0]); int(p[1]); return True
        except: return False

    ai_valid = {n: _is_valid_ai(r) for n, r in [("gpt", gpt_r), ("grok", grok_r), ("gemini", gemini_r), ("claude", claude_r)]}
    ...
    if sp_h > 1 and sp_d > 1 and sp_a > 1:
        ...
        shin_h, shin_d, shin_a = (1/sp_h)/margin*100, (1/sp_d)/margin*100, (1/sp_a)/margin*100
    ...
    if sharp_detected:
        for s in smart_signals:
            if "Sharp" in str(s) or "sharp" in str(s):
                if re.search(r"(主胜|主队|走主|资金\s*主)", str(s)): sharp_dir = "home"; break
                elif re.search(r"(客胜|客队|走客|资金\s*客)", str(s)): sharp_dir = "away"; break
                elif re.search(r"(平局|走平|资金\s*平)", str(s)): sharp_dir = "draw"; break

    steam_type, steam_dir = None, None
    for s in smart_signals:
        if "Steam" in str(s):
            steam_type = "reverse" if "反向" in str(s) or "未跟" in str(s) else "normal"
            if re.search(r"(主胜|主)", str(s)): steam_dir = "home"; break
            elif re.search(r"(客胜|客)", str(s)): steam_dir = "away"; break

    vote = match_obj.get("vote", {})
    vote_hot_dir, vote_hot_pct = None, 0
    try:
        vh, vd, va = int(vote.get("win", 33) or 33), int(vote.get("same", 33) or 33), int(vote.get("lose", 33) or 33)
        vote_hot_pct = max(vh, vd, va)
        if vote_hot_pct >= 55:
            vote_hot_dir = "home" if vh == vote_hot_pct else ("draw" if vd == vote_hot_pct else "away")
    except: pass

    dupan_detected, dupan_true_dir, dupan_confirm = False, None, 0
    if sharp_detected and sharp_dir and sharp_dir != shin_dir:
        if vote_hot_dir == shin_dir and vote_hot_pct >= 55: dupan_confirm += 3
        if steam_dir == sharp_dir: dupan_confirm += 3 if steam_type == "reverse" else 2
        if any("冷门" in str(s) for s in smart_signals): dupan_confirm += 2
        if dupan_confirm >= 3: dupan_detected, dupan_true_dir = True, sharp_dir

    hp_eng = engine_result.get("home_prob", 33)
    ap_eng = engine_result.get("away_prob", 33)
    hot_side = "home" if hp_eng > ap_eng else "away"
    pre_pred = {"home_win_pct": hp_eng, "away_win_pct": ap_eng, "steam_move": stats.get("steam_move", {}), "smart_signals": smart_signals}
    cold_door = ColdDoorDetector.detect(match_obj, pre_pred)

    # -------- 🎯 数学降维 1: 贝叶斯质心漂移 (Bayesian Drift) --------
    ...
    for h in range(9):
        for a in range(9):
            ...
            sc = f"{h}-{a}"
            if h > 5 or a > 5: sc = "胜其他" if h > a else ("负其他" if a > h else "平其他")
            drifted_probs[sc] = drifted_probs.get(sc, 0.0) + (ph * pa * 100)

    # -------- 🎯 数学降维 2: AI 特征提取 (解决选票稀释) --------
    ...
    # ====================================================================
    # 🚨 终极护栏：绝对方向控制，斩杀一切0-1配主胜的UI错位bug 🚨
    # ====================================================================
    final_score = ranked[0][0] if ranked else "1-1"
    
    def _dir_of_sc(sc_str):
        try:
            if "胜" in sc_str: return "home"
            if "平" in sc_str: return "draw"
            if "负" in sc_str: return "away"
            h, a = map(int, sc_str.split("-"))
            return "home" if h > a else ("away" if h < a else "draw")
        except: return None

    # 如果 Softmax 算出的总概率面倒向 home，但单项比分最高的是 away(比如0-1)
    if _dir_of_sc(final_score) != final_direction:
        for sc, _ in ranked:
            if _dir_of_sc(sc) == final_direction:
                print(f"    🚨 [铁血护栏] 大盘方向({final_direction})与最高分比分({final_score})冲突！强制修正为: {sc}")
                final_score = sc
                break
    ...
    if valid_ai_count == 0: avg_ai_conf = 50.0
        ...
        dirs = [_dir_of_sc(r.get("ai_score","")) for r in all_ai.values() if _dir_of_sc(r.get("ai_score",""))]
    ...
    return {
        ...
        "smart_signals": sigs,
        "expected_total_goals": round(final_mu_h + final_mu_a, 2),
...
def extract_num(ms):
    base = next((v for k, v in {"一":1000, "二":2000, "三":3000, "四":4000, "五":5000, "六":6000, "日":7000, "天":7000}.items() if k in str(ms)), 0)
...
def run_predictions(raw, use_ai=True):
    ms = raw.get("matches", [])
    print("\n" + "=" * 80)
    print(f"  [vMAX 17.0 终极典藏版] 质心漂移 + Softmax + 铁血护栏全覆盖 | {len(ms)} 场")
    print("=" * 80)
    ...
    diary["reflection"] = f"vMAX17.0 典藏版 | {cold_count}冷门 {others_count}胜其他 {sharp_count}Sharp | 铁血护栏接入"
    save_ai_diary(diary)

    return res, t4

if __name__ == "__main__":
    logger.info("vMAX 17.0 终极版启动")
    print("✅ vMAX 17.0 终极版已加载 — 贝叶斯质心漂移·多智能体编排·Softmax连续面·铁血一致性护栏")
