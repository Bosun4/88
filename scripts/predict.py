# -*- coding: utf-8 -*-
"""
vMAX 18.4.7 — PURE RAW-AI + Claude压缩终审稳定版
============================================================
设计边界：
...
7. 默认优先走统一 OpenAI-compatible 中转：API_KEY / API_URL；未配置时回退各模型独立变量。
8. singleflight + cache：同批次重复触发时不重复扣费；但 0/4 AI 失败不写缓存。
9. Phase1 默认把完整 normalized match JSON 作为 raw_match_full_json 喂给 GPT/Grok/Gemini。
10. Claude Phase2 默认走压缩终审，不再重复塞完整 raw_match_full_json，避免 60k+ prompt 触发中转断流。
...
推荐环境变量：
    ...
    AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID=true
    AI_MIN_PHASE1_VALID_FOR_CLAUDE=2
    AI_USE_COMPACT_CLAUDE_AUDIT=true
    AI_MAX_PHASE1_REASON_CHARS_FOR_CLAUDE=350
    AI_CLAUDE_COMPACT_MATCH_CHAR_LIMIT=1800
    AI_CLAUDE_PROMPT_CHAR_LIMIT=32000
...
ENGINE_ARCHITECTURE = (
    "PURE RAW-AI: Phase1完整抓包raw_match_full_json + 体彩竞彩赔率标注 + "
    "AI自主联网核对欧洲主流欧赔 + Sharp/聪明钱方向识别 + 联赛/球队风格比分约束 + "
    "AI direction_probs优先 + 历史经验审计卡强制回应 + 四层比分结构(direction/goal_band/btts/score) + "
    "方向精选分/比分形态分双评分 + GPT/Grok/Gemini初审 + Claude压缩终审；"
    "无CRS/无本地矩阵/AI失败即弃权 + 持久化缓存/磁盘锁防重复扣费"
)
...
AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID = _env_bool("AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID", True)
AI_MIN_PHASE1_VALID_FOR_CLAUDE = max(0, _env_int("AI_MIN_PHASE1_VALID_FOR_CLAUDE", 2))
AI_USE_COMPACT_CLAUDE_AUDIT = _env_bool("AI_USE_COMPACT_CLAUDE_AUDIT", True)
AI_MAX_PHASE1_REASON_CHARS_FOR_CLAUDE = max(80, _env_int("AI_MAX_PHASE1_REASON_CHARS_FOR_CLAUDE", 350))
AI_CLAUDE_COMPACT_MATCH_CHAR_LIMIT = max(600, _env_int("AI_CLAUDE_COMPACT_MATCH_CHAR_LIMIT", 1800))
AI_CLAUDE_PROMPT_CHAR_LIMIT = max(12000, _env_int("AI_CLAUDE_PROMPT_CHAR_LIMIT", 32000))
...
def _compact_text(v: Any, limit: int) -> str:
    if v is None:
        return ""
    if isinstance(v, (dict, list)):
        s = _json_compact(v, limit)
    else:
        s = str(v)
...
class ExperienceAuditEngine:
    """旧版 ExperienceEngine 的 prompt-only 迁移版。只生成经验审计卡，不修改概率/方向/比分。"""
    ...
    def _has_derby_or_rival(self, match_data: Dict[str, Any]) -> str:
    ...
    def analyze(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        derby = self._has_derby_or_rival(match_data)
        if derby:
...
def _compact_experience_cards(exp: Dict[str, Any], limit_cards: int = 6) -> List[Dict[str, Any]]:
    rows = exp.get("triggered", []) if isinstance(exp, dict) else []
    out = []
    for t in rows[:limit_cards]:
        out.append({
            "id": t.get("id"),
            "name": t.get("name"),
            "weight": t.get("weight"),
            "reason": t.get("reason"),
            "question": t.get("ai_question"),
        })
    return out
...
def build_phase1_prompt(match_analyses: List[Dict[str, Any]]) -> str:
    ...
    p += "4. 必须识别 Sharp/聪明钱方向。若热门方向很热但赔率不降反升，或弱方赔率逆势下压，应判断是否存在反向资金、造热、阻上或诱盘，但不能无证据反指。\n"
    p += "5. 必须结合联赛风格和球队风格给比分。低节奏/防守型倾向小比分，强控球强压迫可能2-0/3-0，高节奏联赛可放大爆冷和双方进球。\n"
    p += "6. 不要默认输出 1-1、2-1、1-0、0-1、1-2；若选择这些比分，必须是从原始字段、Sharp方向、联赛/球队风格和联网核对中独立推导。\n"
    p += '7. top3 必须是比分分布，不要只给方向。top3[0].score 必须与 final_direction 一致。必须额外输出 direction_probs={"home":数字,"draw":数字,"away":数字}。\n'
    p += "8. 必须按四层结构输出：final_direction → goal_band(0-1/2/3/4+) → btts(yes/no/unclear) → top3比分。比分必须与 goal_band 和 btts 自洽。\n"
    p += "9. 必须说明为什么不选另外两个方向；若 top1 是 1-2，说明为什么不是 0-1/0-2；若 top1 是 2-1，说明为什么不是 1-0/2-0；若 top1 是 1-1，说明为什么不是 0-0/2-2。\n"
    p += "10. 0-1、0-2、0-3 是合法客胜比分；其他比分可输出 4-3、3-4 等精确比分。\n"
    p += "11. 如存在 experience_audit_cards，必须在 audit.experience_review 中逐条说明 accepted/rejected/neutral + 理由；但最终比分仍由原始赔率、联网欧赔、Sharp、联赛/球队风格共同决定。\n"
    p += "</task_rules>\n\n"
...
def _compact_match_for_claude(idx: int, ma: Dict[str, Any]) -> Dict[str, Any]:
    m = ma["match"]
    exp = ma.get("experience_audit") or _experience_engine().analyze(m)
    ...
    base = {
        "match": idx,
        "teams": f"{m.get('home_team', m.get('home', 'Home'))} vs {m.get('away_team', m.get('guest', 'Away'))}",
        "league": m.get("league", m.get("cup", "")),
        ...
        "total_goals_a0_a7": {f"a{i}": m.get(f"a{i}", "") for i in range(8)},
        "hftf": hf_l,
        "change": m.get("change", {}),
        "vote": m.get("vote", {}),
        "experience_audit_cards": _compact_experience_cards(exp, 8),
        "style_extra": {k: m.get(k) for k in (
            "league_style", "league_profile", "team_style", "home_style", "away_style",
            "home_form", "away_form", "weather", "injury", "lineup", "news", "motivation", "schedule",
        ) if m.get(k) not in (None, "", {}, [])},
        "information": _compact_text(m.get("information", {}), 420),
        "points": _compact_text(m.get("points", {}), 420),
        "external_context": _compact_text(ma.get("external_context", {}), 350),
    }

    s = _json_compact(base, AI_CLAUDE_COMPACT_MATCH_CHAR_LIMIT)
    try:
        return json.loads(s)
    except Exception:
        return {"match": idx, "compact": s}


def _compact_phase1_item(r: Dict[str, Any], idx: int, ai: str) -> Dict[str, Any]:
    if not isinstance(r, dict) or not r:
        return {"match": idx, "ai": ai, "abstain": True}
    ...
    return {
        ...
        "reason": _compact_text(r.get("reason", ""), AI_MAX_PHASE1_REASON_CHARS_FOR_CLAUDE),
        "audit": {
            "web_odds_check": audit.get("web_odds_check", ""),
            "european_odds": _compact_text(audit.get("european_odds", ""), 180),
            "market_divergence": _compact_text(audit.get("market_divergence", ""), 180),
            "sharp_money_direction": audit.get("sharp_money_direction", ""),
            "sharp_evidence": _compact_text(audit.get("sharp_evidence", ""), 220),
            "league_style": _compact_text(audit.get("league_style", ""), 180),
            "team_style": _compact_text(audit.get("team_style", ""), 180),
            "style_score_logic": _compact_text(audit.get("style_score_logic", ""), 220),
            "direction_rejection": _compact_text(audit.get("direction_rejection", ""), 220),
            "experience_review": _compact_text(audit.get("experience_review", ""), 260),
        },
        "data_missing": r.get("data_missing", [])[:8] if isinstance(r.get("data_missing", []), list) else [],
    }


def build_claude_compact_final_audit_prompt(
    match_analyses: List[Dict[str, Any]],
    phase1_results: Dict[str, Dict[int, Dict[str, Any]]],
) -> str:
    matches_pack = [_compact_match_for_claude(i, ma) for i, ma in enumerate(match_analyses, 1)]

    phase1_pack = []
    for idx in range(1, len(match_analyses) + 1):
        row = {"match": idx, "gpt": None, "grok": None, "gemini": None}
        for ai in PHASE1_NAMES:
            row[ai] = _compact_phase1_item((phase1_results.get(ai, {}) or {}).get(idx, {}), idx, ai)
        phase1_pack.append(row)

    p = "<final_audit_context>\n"
    p += "你是 Claude 最终 RAW-AI 主裁。当前是压缩终审模式：你不会再收到完整 raw_match_full_json，只收到每场关键体彩竞彩赔率、变盘、热度、总进球、半全场、经验审计卡摘要，以及 GPT/Grok/Gemini 初审压缩结果。\n"
    p += "禁止为了体现审计而反对初审。若三家初审方向/比分高度一致，除非关键赔率/总进球/BTTS/经验审计卡存在硬冲突，不得无证据乱改。\n"
    p += "必须明确区分：match_data_compact 中赔率是体彩竞彩抓包赔率，不是欧洲平均欧赔。若无联网能力，audit.web_odds_check 写 web_search_unavailable，不能假装查过。\n"
    p += "experience_audit_cards 只是审计问题，不是本地裁决。你必须在 audit.experience_review 中逐条写 accepted/rejected/neutral + 理由。\n"
    p += "必须输出四层比分结构：final_direction、direction_probs、goal_band、btts、top3。top3[0].score 必须与 final_direction、goal_band、btts 自洽。\n"
    p += "如果改动 Phase1 共识，必须说明硬依据：体彩1X2/让球、变盘、散户热度、总进球a0-a7、半全场、Sharp方向、联赛/球队风格或经验审计卡冲突。\n"
    p += "禁止引用 CRS、贝叶斯、本地矩阵、本地风控编号、固定常见比分模板。\n"
    p += "</final_audit_context>\n\n"

    p += "<final_output_rule>\n"
    p += "严格输出 JSON 数组。每场一个对象：\n"
    p += '{"match":1,"final_direction":"home/draw/away","direction_probs":{"home":45,"draw":28,"away":27},"goal_band":"0-1/2/3/4+","btts":"yes/no/unclear","top3":[{"score":"2-0","prob":0.18,"market_logic":"..."}],"reason":"...","ai_confidence":0-100,"risk_level":"low/medium/high","data_missing":[],"audit":{"odds_source":"体彩竞彩抓包赔率","web_odds_check":"searched/web_search_unavailable/european_odds_missing","european_odds":"...","market_divergence":"...","sharp_money_direction":"home/draw/away/home_or_draw/away_or_draw/unclear","sharp_evidence":"...","league_style":"...","team_style":"...","style_score_logic":"...","direction_rejection":"...","total_goals":"...","money_flow":"...","experience_review":"D01:neutral because ...; X01:accepted because ..."}}\n'
    p += "禁止 markdown，禁止 JSON 外文本。\n"
    p += "</final_output_rule>\n\n"

    p += "<match_data_compact>\n"
    p += json.dumps(matches_pack, ensure_ascii=False, separators=(",", ":"), default=str)
    p += "\n</match_data_compact>\n\n"

    p += "<phase1_ai_results_compact>\n"
    p += json.dumps(phase1_pack, ensure_ascii=False, separators=(",", ":"), default=str)
    p += "\n</phase1_ai_results_compact>\n"

    if len(p) > AI_CLAUDE_PROMPT_CHAR_LIMIT:
        over = len(p)
        p = p[:AI_CLAUDE_PROMPT_CHAR_LIMIT]
        p += f"\n...[CLAUDE_PROMPT_TRUNCATED {over}->{AI_CLAUDE_PROMPT_CHAR_LIMIT}]"
        p += "\n请仍然严格输出 JSON 数组，不要输出解释性文本。"
...
def build_claude_final_audit_prompt(
    ...
    if AI_USE_COMPACT_CLAUDE_AUDIT:
        return build_claude_compact_final_audit_prompt(match_analyses, phase1_results)
    ...
    for ai in PHASE1_NAMES:
        ...
        for idx in range(1, len(match_analyses) + 1):
            r = rs.get(idx)
            if r:
                p += json.dumps(_compact_phase1_item(r, idx, ai), ensure_ascii=False) + "\n"
            else:
                p += f"[{idx}] 弃权\n"
        p += f"</{ai}>\n"
    p += "</phase1_ai_results>\n\n"
...
def get_ai_key(ai_name: str) -> str:
    common = _clean_env_key("API_KEY")
    if common:
        return common
    u = ai_name.upper()
    if ai_name == "gpt":
        return _clean_env_key("GPT_API_KEY", "OPENAI_API_KEY", "API_KEY")
    return _clean_env_key(f"{u}_API_KEY", f"{u}_KEY", "API_KEY")


def get_ai_url(ai_name: str) -> str:
    common = _clean_env_url("API_URL", "BASE_URL")
    if common:
        return common
    u = ai_name.upper()
    if ai_name == "gpt":
        return _clean_env_url("GPT_API_URL", "OPENAI_API_URL", "API_URL", "BASE_URL")
    return _clean_env_url(f"{u}_API_URL", f"{u}_URL", "API_URL", "BASE_URL")
...
def get_common_url() -> str:
    return _clean_env_url("API_URL", "BASE_URL", "GPT_API_URL", "OPENAI_API_URL", "GROK_API_URL", "GEMINI_API_URL", "CLAUDE_API_URL")
...
def debug_ai_config() -> None:
    print(f"[COMMON GATEWAY] API_URL={_clean_env_url('API_URL', 'BASE_URL') or '<model-specific>'} API_KEY={_mask_key(_clean_env_key('API_KEY')) or '<model-specific>'}")
    for n in AI_NAMES:
        print(f"[AI CONFIG] {n.upper()} model={_model_for(n)} url={get_ai_url(n) or '<missing>'} key={_mask_key(get_ai_key(n))}")
    print(
        f"[AI MODE] pure_raw=True singleflight={AI_SINGLEFLIGHT_ENABLED} cache_ttl={AI_DECISION_CACHE_TTL} "
        f"max_req_per_ai={AI_MAX_REQUESTS_PER_AI} external={ENABLE_EXTERNAL_CONTEXT} "
        f"claude_compact={AI_USE_COMPACT_CLAUDE_AUDIT} claude_min_phase1={AI_MIN_PHASE1_VALID_FOR_CLAUDE}"
    )
...
def _parse_ai_json(raw_text: str, num_matches: int, ai_name: str = "") -> Dict[int, Dict[str, Any]]:
    ...
    for pos, item in enumerate(arr, 1):
        ...
        final_direction = _normalize_direction(
            item.get("final_direction", item.get("direction", item.get("result", ""))),
            top_score,
        )
...
async def async_call_one_ai_batch(
    ...
    key = get_ai_key(ai_name)
    base_url = get_ai_url(ai_name)
    model = _model_for(ai_name)

    AI_CALL_STATUS[ai_name] = {"ok": False, "status": "init", "model": model, "count": 0, "requests": 0}

    if not key:
        print(f"  [{ai_name.upper()}] no_key: 检查 API_KEY / {ai_name.upper()}_API_KEY 等环境变量")
        AI_CALL_STATUS[ai_name].update({"status": "no_key"})
        return ai_name, {}, "no_key"

    if not base_url:
        print(f"  [{ai_name.upper()}] no_url: 检查 API_URL / {ai_name.upper()}_API_URL 等环境变量")
    ...
    for req_no in range(1, AI_MAX_REQUESTS_PER_AI + 1):
        ...
        try:
            timeout = aiohttp.ClientTimeout(
                ...
                sock_connect=AI_CONNECT_TIMEOUT,
                sock_read=AI_READ_TIMEOUT,
            )
            ...
            async with session.post(url, headers=headers, json=payload, timeout=timeout) as r:
                ...
                if r.status != 200:
                    ...
                    AI_CALL_STATUS[ai_name].update({"status": f"http_{r.status}", "http_error": text_for_error[:800]})
                    continue
            ...
            print(f"    {ai_name.upper()} 调用异常: {str(e)[:200]}")
...
def _stable_ai_cache_key(match_analyses: List[Dict[str, Any]], phase: str = "pure") -> str:
    ...
    raw = json.dumps(
        {"version": ENGINE_VERSION, "phase": phase, "matches": compact},
        ensure_ascii=False,
        sort_keys=True,
        default=str,
    )
...
async def _run_ai_matrix_two_phase_inner(
    ...
    print(f"  [v18.4 Phase1 Prompt] {len(prompt):,}字符 → GPT/Grok/Gemini | common_gateway={bool(_clean_env_key('API_KEY') and _clean_env_url('API_URL'))}")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        valid_phase1_models = [n for n in PHASE1_NAMES if all_results.get(n)]
        valid_phase1_count = len(valid_phase1_models)

        if AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID and valid_phase1_count < AI_MIN_PHASE1_VALID_FOR_CLAUDE:
            print(
                f"  [Claude SKIP] Phase1有效模型数={valid_phase1_count}/{len(PHASE1_NAMES)} "
                f"< AI_MIN_PHASE1_VALID_FOR_CLAUDE={AI_MIN_PHASE1_VALID_FOR_CLAUDE}，跳过Claude避免白花token"
            )
            AI_CALL_STATUS["claude"] = {
                "ok": False,
                "status": "skipped_phase1_insufficient",
                "model": _model_for("claude"),
                "count": 0,
                "requests": 0,
                "phase1_valid": valid_phase1_models,
            }
            all_results["claude"] = {}
        else:
            ...
            cl_name, cl_res, _ = await async_call_one_ai_batch(
                session,
                audit_prompt,
                num,
                "claude",
                _phase_system("claude"),
            )
...
if __name__ == "__main__":
    ...
    print("   模式: AI成功=AI直出；AI失败=弃权；无本地兜底；Claude默认压缩终审")