...
def call_gemini(p):
    print("  [Gemini 链路启动]")
    return call_model(p, GEMINI_API_URL, GEMINI_API_KEY, [GEMINI_MODEL] if 'GEMINI_MODEL' in globals() else ["gemini-1.5-pro", "gemini-pro"])

# ==================== 完整还原：融合中枢 (前端要的字段一个不差) ====================
def merge_all(gpt, gemini, stats):
    ...
    return {
        ...
        "value_bets_summary": v_tags, "home_ev": val_h, "draw_ev": val_d, "away_ev": val_a,
...
def run_predictions(raw):
    ...
    for i, m in enumerate(ms):
        ...
        print("    Poisson:%s MC:%s RF:H%.0f%% GB:H%.0f%% NN:H%.0f%% Consensus:%d/%d" % (
            sp["poisson"].get("predicted_score", "?"), 
            sp["monte_carlo"].get("top_scores",[{}])[0].get("score","?") if "monte_carlo" in sp and sp["monte_carlo"].get("top_scores") else "?",
