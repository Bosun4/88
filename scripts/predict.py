...
【临场资金时间序列升降级协议】
目标：把比分判断从静态低赔锚升级为“多时间点 + 多公司 + 多市场闭环”的动态读盘；禁止泊松，禁止机械锚定最低赔比分。
1. 时间点：若具备联网或外部盘口能力，必须尽量收集 初盘 / T-24h / T-6h / T-90m / T-60m / T-30m / 临场最终；若缺任一时间点，必须说明 missing_timeline_points，不得编造。
2. 公司角色：至少尝试区分 百家平均、竞彩官方、威廉、澳门、Bet365、Pinnacle、立博。Pinnacle/Bet365/澳门的亚盘与大小球最后变化优先级高于早盘正确比分低赔；竞彩官方用于识别区域风控与大众入口。
3. 四市场闭环：欧赔负责方向与分流，亚洲让球负责赢球深度/不败保护，大小球负责总进球边界，BTTS/零封/球队进球数负责双方进球结构，正确比分只做最后映射，不得单独主导。
4. 聪明钱联动：最后 T-60m/T-30m 若出现跨市场同步（例如主队平手低水持续压低 + 大球水下压 + 客水抬高），必须视为临场资金确认或风险回补，并显式判断它是确认、诱导、分流还是回补。
5. 比分升降级：若原主线为 1-1/0-0 等低分，但临场主队低水保护且大球水下压、小球水抬升，应把 2-1/1-2 等 3球边界比分从尾部升权；若让球升深且大小压低、BTTS/零封一致，应把 2-0/0-2/3-0/0-3 升权而不是追 2-1。
6. 反证条件：每场必须写 minimum_evidence_needed / why_this_can_fail；明确哪些 T-30m 盘口/水位变化会推翻当前主线，哪些比分因此降级或升权。
7. 输出要求：market_interpretation 或 reason 中必须体现“时间序列角色判断 + 四市场闭环 + 比分升降级”；若没有真实时间序列数据，只能写 timeline_unavailable，并降级信心。
""".strip()
