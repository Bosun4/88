...
def build_phase1_prompt(match_analyses):
    ...
    p += "【反常识思维】不要因为保守就选小比分 1-1/1-0，体彩经常开出 3-1, 3-0, 3-2, 4-1 等大比分！\n\n"
    ...
    p += "\n【特别要求】\n"
    p += "严重警告：遇到强弱悬殊且期望进球大于 2.8 的比赛，绝不要缩手缩脚选小比分，必须给出符合比赛风格的大比分，可以根据进球数进行判断落点，比如4球区间在4.3倍-5.25倍区间 5球6-7.5倍左右 7球在23倍以内！0球在9.5倍以内大概率小球或者 0-0 以此类推可以给出相应的比分比如4球强弱 3-1 虐菜 4-0 强强 2-2 具体根据实际情况进行调整 \n"
...
def build_phase2_prompt(match_analyses, phase1_results):
    ...
    p += "【强引导原则】\n"
    p += "① 若其他AI给出比如p3-1/2-3/1-3 等高赔比分，且λ支持大球，你必须跟随或者放大，不要保守拉回 1-1/2-1！\n"
    p += "② 不要惧怕高赔。体彩经常爆出高赔率20+的比分。\n\n"
...
async def async_call_one_ai_batch(session, prompt, url_env, key_env, models_list, num_matches, ai_name):
    ...
    AI_PROFILES = {
        ...
        "grok": {"sys": "你是Grok，具备实时联网能力。大胆预测比分根据sharp方向判断方向输赢以及是否冷门再根据亚盘判断大小球以及进球数再给出合理的比分比如(2-1/1-3/3-0/2-2)具体可以敏感的判断进球数来合理分配符合比赛风格的比分。直接输出JSON数组，不要任何废话。", "temp": 0.25},
        "gpt": {"sys": "你是激进派量化足球分析师。纯数学计算TOP3。直接输出JSON数组，不要任何前缀说明。", "temp": 0.20},
        "gemini": {"sys": "你是足球分析引擎。严格执行数学计算。直接输出JSON数组，不要Markdown标记。", "temp": 0.22},
    }
...
async def run_ai_matrix_two_phase(match_analyses):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        _, claude_r, _ = await async_call_one_ai_batch(
            ...
            ["熊猫-按量-顶级特供-官max-claude-opus-4.7-thinking", "熊猫特供-超纯满血-99额度-claude-opus-4.6-thinking"], num, "claude"
        )
