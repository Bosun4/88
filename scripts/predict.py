#!/usr/bin/env python3
"""
predict.py vMAX 3.3 — 冷门猎手·深度赔率映射版
集成: 原版全部逻辑 + ColdDoorDetector + calibrate_realistic_score + 4层try/except
"""
    ...
    def detect(match, prediction):
        ...
        try:
            ...
            if max_vote >= 65:
                ...
                signals.append(f"⚠️ 散户偏向{max_vote}%")
                strength += 2
        except: pass
        ...
        sp_a = float(match.get("sp_away", match.get("lose", 0)) or 0)
        if sp_h > 1:
            ...
            if abs(implied_h - hp) > 15 and hp > 58:
                ...
                strength += 4
        if sp_a > 1:
            implied_a = 100 / sp_a * 0.92
            if abs(implied_a - ap) > 15 and ap > 58:
                signals.append(f"❄️ 客胜赔率vs模型背离{abs(implied_a-ap):.0f}%")
                strength += 4
...
def calibrate_realistic_score(current_score, sp_h, sp_d, sp_a, prediction):
    """赔率映射+冷门强度联合校准：防止输出不合理的高比分"""
    cold = prediction.get("cold_door", {})
    if cold.get("is_cold_door") and cold.get("level") == "顶级":
        return current_score  # 顶级冷门允许原预测

    implied_total = (1/sp_h + 1/sp_d + 1/sp_a) * 100 if sp_h > 1 and sp_d > 1 and sp_a > 1 else 300

    if implied_total < 280:
        allowed = REALISTIC_MAP["ultra_low"]
    elif implied_total < 310:
        allowed = REALISTIC_MAP["low"]
    elif implied_total < 340:
        allowed = REALISTIC_MAP["medium"]
    else:
        allowed = REALISTIC_MAP["high"]
    ...
    try:
        ...
        if home + away <= 2:
            ...
            return "2-1" if home >= away else "1-2"
        else:
            return "2-1" if home >= away else "1-2"
    except:
        return "1-1"
...
def build_batch_prompt(match_analyses):
    ...
    for i, ma in enumerate(match_analyses):
        ...
        smart_str = ", ".join(smart_sigs[:3]) if smart_sigs else "无收割信号"
    ...
    p += "【严格输出格式示例】\n"
    p += '[{"match":1,"score":"0-2","reason":"散户正被主队虚假连胜基本面洗脑疯狂送钱。而隐性xG暴露出主队仅0.8真实攻击力，数据严重倒挂。配合亚盘深让死亡陷阱，这是庄家精心布置的吸筹绞肉机。资本正无底线诱多做局喂毒，逆向思维直指客队，血洗主胜筹码。","ai_confidence":87,"value_kill":true,"dark_verdict":"客队屠杀主胜筹码，庄家笑到最后"}]'
    return p

# ====================================================================
# AI矩阵
# ====================================================================
FALLBACK_URLS = [None, "https://api520.pro/v1", "https://www.api520.pro/v1", "https://api521.pro/v1", "https://www.api521.pro/v1", "https://api522.pro/v1", "https://www.api522.pro/v1", "https://69.63.213.33:666/v1", "https://api523.pro/v1", "https://api524.pro/v1"]
        ...
        for mn in sorted(models_list, key=lambda x: success_rate[x], reverse=True):
            for base_url in urls:
                ...
                if is_gem:
                    ...
                    payload = {"contents": [{"parts": [{"text": prompt}]}], "generationConfig": {"temperature": 0.12, "topP": 0.9}, "systemInstruction": {"parts": [{"text": "必须严格输出JSON数组。reason必须是毫无道德底线的暗黑杀猪盘解剖，使用极度恶毒黑话，字数60-110字，冷血傲慢，句号结尾。"}]}}
                else:
                    headers["Authorization"] = f"Bearer {key}"
                    payload = {"model": mn, "messages": [{"role": "system", "content": "你是一个毫无道德底线、极度冷血的暗网终极操盘手。只输出纯JSON数组，无任何多余文字。reason字段必须用最恶毒黑话揭穿庄家血洗散户的杀局，限60-110字，极度鄙视散户，句号结束。"}, {"role": "user", "content": prompt}], "temperature": 0.12}
                                    ...
                                    if isinstance(arr, list):
                                        for item in arr:
                                            if item.get("match") and item.get("score"):
                                                results[item["match"]] = {"ai_score": item.get("score"), "analysis": str(item.get("reason", "")).strip()[:200], "ai_confidence": int(item.get("ai_confidence", 60)), "value_kill": bool(item.get("value_kill", False)), "dark_verdict": str(item.get("dark_verdict", ""))}
                                except: pass
                            if len(results) >= max(1, num_matches * 0.5):
                                print(f"    ✅ {ai_name.upper()} 压榨成功: {len(results)}/{num_matches} (模型: {mn[:25]})")
                                return ai_name, results, mn
                            else: print(f"    ⚠️ 解析不足，切换...")
                        elif r.status == 429:
                            await asyncio.sleep(2 ** attempt * 5); continue
                        else: print(f"    ⚠️ HTTP {r.status}")
                except asyncio.TimeoutError: print(f"    ⏰ 超时")
                except Exception as e: print(f"    ⚠️ {str(e)[:50]}")
                await asyncio.sleep(0.4)
        await asyncio.sleep(1.5)
    print(f"    ❌ {ai_name.upper()} 所有线路压榨至死！")
    return ai_name, {}, "failed"

async def run_ai_matrix(prompt, num_matches):
    ai_configs = [
        ("claude", "CLAUDE_API_URL", "CLAUDE_API_KEY", ["熊猫-按量-顶级特供-官max-claude-opus-4.6-thinking", "熊猫-按量-满血copilot-claude-opus-4.6-thinking", "熊猫-按量-特供顶级-官方正向满血-claude-opus-4.6-thinking"]),
        ("grok", "GROK_API_URL", "GROK_API_KEY", ["熊猫-A-7-grok-4.2-多智能体讨论", "熊猫-A-6-grok-4.2-thinking"]),
        ("gpt", "GPT_API_URL", "GPT_API_KEY", ["熊猫-A-7-gpt-5.4", "熊猫-按量-gpt-5.3-codex-满血", "熊猫-A-10-gpt-5.3-codex"]),
    ...
    for res in results:
        ...
        else: print(f"  [CRITICAL] AI崩溃: {res}")
    return all_results
...
def merge_result(engine_result, gpt_r, grok_r, gemini_r, claude_r, stats, match_obj):
    ...
    # ========== 冷门信号识别 ==========
    pre_prediction = {"home_win_pct": fhp, "draw_pct": fdp, "away_win_pct": fap, "steam_move": stats.get("steam_move", {}), "smart_signals": stats.get("smart_signals", []), "line_movement_anomaly": stats.get("line_movement_anomaly", {})}
    cold_door = ColdDoorDetector.detect(match_obj, pre_prediction)

    # 冷门信号注入smart_signals
    sigs = list(stats.get("smart_signals", []))
    if cold_door["is_cold_door"]:
        sigs.extend(cold_door["signals"])
        cf = max(30, cf - 5)  # 冷门比赛降低置信度

    # ========== 比分现实校准 ==========
    temp_pred = {"cold_door": cold_door}
    final_score = calibrate_realistic_score(final_score, sp_h, sp_d, sp_a, temp_pred)
...
def select_top4(preds):
    for p in preds:
        ...
        if exp_info.get("total_score", 0) >= 15 and pr.get("result") == "平局" and exp_info.get("draw_rules", 0) >= 3: s += 12
        elif exp_info.get("total_score", 0) >= 10: s += 5
        if exp_info.get("recommendation", "").startswith("⚠️"): s -= 3
        smart_money = str(pr.get("smart_money_signal", "")); direction = pr.get("result", "")
...
def run_predictions(raw, use_ai=True):
    ...
    if use_ai and match_analyses:
        ...
        print(f"  [PROMPT] {len(prompt):,} 字符 | 开始压榨AI矩阵...")
        start_t = time.time()
        all_ai = asyncio.run(run_ai_matrix(prompt, len(match_analyses)))
        print(f"  [AI MATRIX] 完成 {time.time()-start_t:.1f}s")
    ...
    for i, ma in enumerate(match_analyses):
        ...
        try: mg = apply_experience_to_prediction(m, mg, exp_engine); print(f"    → experience_rules 已注入")
        except Exception as e: print(f"    ⚠️ experience_rules跳过: {e}")
        try: mg = apply_odds_history(m, mg); print(f"    → odds_history 已注入")
        except Exception as e: print(f"    ⚠️ odds_history跳过: {e}")
        try: mg = apply_quant_edge(m, mg); print(f"    → quant_edge 已注入")
        except Exception as e: print(f"    ⚠️ quant_edge跳过: {e}")
        try: mg = apply_wencai_intel(m, mg); print(f"    → wencai_intel 已注入")
        except Exception as e: print(f"    ⚠️ wencai_intel跳过: {e}")
        try: mg = upgrade_ensemble_predict(m, mg); print(f"    → advanced_models 已注入")
    ...
    diary["reflection"] = f"vMAX3.3冷门猎手模式 | 检测到{cold_count}场冷门信号 | 5层增强管线运行"
    save_ai_diary(diary)
    return res, t4