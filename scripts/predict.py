# -*- coding: utf-8 -*-
"""
vMAX 18.4.7 — PURE RAW-AI + Claude 600秒等待修复版
============================================================
设计边界：
...
9. 默认把完整 normalized match JSON 作为 raw_match_full_json 喂给 Phase1；Claude 默认启用压缩终审，避免长 prompt 被中转断开。
10. 优先解析 AI 输出的 direction_probs；没有时才用 top3 方向占比做前端兼容。
11. 时间配置：GPT/Grok/Gemini 默认等待 300 秒；Claude 默认等待 600 秒。
...
ENGINE_ARCHITECTURE = (
    ...
    "GPT/Grok/Gemini初审 + Claude压缩终审/600秒等待；无CRS/无本地矩阵/AI失败即弃权 + "
    "持久化缓存/磁盘锁防重复扣费"
)
...
# 核心修改：其他模型默认 300 秒，Claude 默认 600 秒。
AI_READ_TIMEOUT = _env_int("AI_READ_TIMEOUT", 300)
AI_CONNECT_TIMEOUT = _env_int("AI_CONNECT_TIMEOUT", 45)
AI_CLAUDE_READ_TIMEOUT = _env_int("AI_CLAUDE_READ_TIMEOUT", 600)
AI_CLAUDE_CONNECT_TIMEOUT = _env_int("AI_CLAUDE_CONNECT_TIMEOUT", 60)

# 默认不额外增加 Claude 请求次数，避免重复扣费。
AI_CLAUDE_MAX_REQUESTS = max(1, _env_int("AI_CLAUDE_MAX_REQUESTS", AI_MAX_REQUESTS_PER_AI))
AI_CLAUDE_RETRY_BACKOFF_SECONDS = max(1, _env_int("AI_CLAUDE_RETRY_BACKOFF_SECONDS", 20))
...
CLAUDE_RAW_PACKET_CHAR_LIMIT = _env_int("CLAUDE_RAW_PACKET_CHAR_LIMIT", 4500)
CLAUDE_MATCH_SUMMARY_CHAR_LIMIT = _env_int("CLAUDE_MATCH_SUMMARY_CHAR_LIMIT", 2200)
...
_RULE_NAME = {
    "D01": "大热必死", "D03": "下游两连胜无三连", "D04": "中游三连胜无四连",
    "D05": "强队六连胜难", "D06": "德比/克星效应", "D08": "强强对话平局率超30%",
    "D09": "九配走平口", "D10": "平手盘水位不动易平", "D11": "半球盘高水诱下盘",
    "D12": "232指数体系平局多", "D13": "攻防数据接近必防平", "D15": "杯赛首轮易平",
    "D16": "中游无欲无求易平", "D18": "主场连败止血易平", "D19": "国际赛均势闷平",
    "U04": "受注比例一边倒反向操作", "U09": "排名差大但盘口便宜", "U10": "赔率剧烈变动",
    "G03": "深盘大小球矛盾", "G08": "0球赔率极低信号", "G09": "7+球赔率极低信号",
    "G10": "CRS 0-0赔率极低", "G11": "双闷队0-0高危", "G12": "双攻队大球高危",
    "B_SHARP": "平局Sharp资金突进", "B_STEAM": "Steam资金方向",
    "M01": "保级队赛季末激战", "M05": "意甲保级财务灾难",
...
def _format_experience_audit_for_prompt(exp: Dict[str, Any]) -> str:
    ...
    p += "这些卡片只作为审计问题；禁止直接改方向、禁止直接改比分、禁止当作本地裁决。AI必须在 audit.experience_review 中说明接受/驳回/中性。\n"
    for t in rows:
...
def _raw_full_packet_line(match_obj: Dict[str, Any], limit: Optional[int] = None) -> str:
    if not INCLUDE_FULL_RAW_PACKET:
        return ""
    use_limit = RAW_PACKET_CHAR_LIMIT if limit is None else limit
    raw_json = _json_compact(match_obj, use_limit if use_limit and use_limit > 0 else 0)
    suffix = ""
    try:
        full_len = len(json.dumps(match_obj, ensure_ascii=False, default=str, separators=(",", ":")))
        if use_limit and use_limit > 0 and full_len > use_limit:
            suffix = f"...[TRUNCATED full_len={full_len} limit={use_limit}]"
    except Exception:
        pass
...
def _prompt_header() -> str:
    ...
    p += "必须自主识别 Sharp/聪明钱方向：重点看赔率下降与散户热度是否同向、弱队/强队身份与盘口阻力是否背离、欧洲欧赔与体彩竞彩是否共振。\n"
    p += "必须结合联赛风格和球队风格给出比分形态。禁止引用或假设任何未给出的本地模型结论。禁止使用 CRS 框架、贝叶斯、本地矩阵、陷阱编号、固定常见比分模板。\n"
    p += "如果没有真实联网/浏览能力，必须在 audit.web_odds_check 写 web_search_unavailable，并在 data_missing 加入 external_european_odds；禁止假装查过欧赔。\n"
    p += "如果出现 experience_audit_cards，它们只用于审计提问，不是本地裁决；必须逐条判断接受/驳回/中性，不得机械跟随。\n"
    p += "</context>\n\n"
    p += "<task_rules>\n"
    ...
    p += "4. 必须识别 Sharp/聪明钱方向；热门方向很热但赔率不降反升，或弱方赔率逆势下压，应判断是否存在反向资金、造热、阻上或诱盘，但不能无证据反指。\n"
    p += "5. 必须结合联赛风格和球队风格给比分：低节奏/防守型倾向小比分，强控球强压迫可能2-0/3-0，高节奏联赛可放大爆冷和双方进球。\n"
    p += "6. 不要默认输出 1-1、2-1、1-0、0-1、1-2；若选择这些比分，必须有当场证据。\n"
...
def _format_one_match_for_prompt(i: int, ma: Dict[str, Any], raw_limit: Optional[int] = None, compact: bool = False) -> str:
    ...
    if compact:
        compact_obj = {
            ...
            "change": m.get("change"),
            "vote": m.get("vote"),
            "a0_a7": {f"a{x}": m.get(f"a{x}") for x in range(8)},
            "information": m.get("information"),
            "points": m.get("points"),
            "style": {
                k: m.get(k)
                for k in ("league_style", "team_style", "home_style", "away_style", "home_form", "away_form", "weather", "injury", "lineup", "motivation", "schedule")
                if k in m
            },
        }
        p += f"compact_raw_match_json:{_json_compact(compact_obj, CLAUDE_MATCH_SUMMARY_CHAR_LIMIT)}\n"
        p += _raw_full_packet_line(m, CLAUDE_RAW_PACKET_CHAR_LIMIT)
    else:
        ...
        p += _raw_field_line(
            "raw_style_extra原始字段",
            {
                ...
                if k in (
                    ...
                    "play_style", "tactical_style", "pace_rating", "tempo", "home_form",
                    "away_form", "weather", "injury", "lineup", "news", "motivation", "schedule"
                )
            },
            FIELD_LIMIT_STYLE_EXTRA,
        )
        p += _raw_full_packet_line(m, raw_limit)
...
def build_phase1_prompt(match_analyses: List[Dict[str, Any]]) -> str:
    ...
    for i, ma in enumerate(match_analyses, 1):
        p += _format_one_match_for_prompt(i, ma, raw_limit=None, compact=False)
    p += "</match_data>\n"
    return p


def build_claude_final_audit_prompt(match_analyses: List[Dict[str, Any]], phase1_results: Dict[str, Dict[int, Dict[str, Any]]]) -> str:
    if not AI_USE_COMPACT_CLAUDE_AUDIT:
        p = "<final_audit_context>\n"
        p += "你是 Claude 最终 RAW-AI 主裁。你看到的是中国体彩竞彩抓包赔率、原始比赛字段、external_context，以及 GPT/Grok/Gemini 初审。你不是反指模型。\n"
        p += "</final_audit_context>\n\n"
        p += build_phase1_prompt(match_analyses)
    else:
        p = "<final_audit_context>\n"
        p += "你是 Claude 最终 RAW-AI 主裁。你看到的是压缩后的中国体彩竞彩抓包字段、经验审计卡、external_context，以及 GPT/Grok/Gemini 初审。\n"
        p += "这次终审使用 compact 模式，目的是避免超长 prompt 导致中转连接重置。你仍必须基于原始赔率结构、Phase1证据、Sharp/聪明钱、联赛/球队风格重新审计。\n"
        p += "你不是反指模型，不需要为了体现审计而反对初审。若三家初审高度一致且没有硬反证，应尊重证据更完整的一侧。\n"
        p += "必须输出 JSON 数组；字段同 phase1；禁止 JSON 外文本。\n"
        p += "</final_audit_context>\n\n"
        p += _prompt_header()
        p += "<compact_match_data>\n"
        for i, ma in enumerate(match_analyses, 1):
            p += _format_one_match_for_prompt(i, ma, raw_limit=CLAUDE_RAW_PACKET_CHAR_LIMIT, compact=True)
        p += "</compact_match_data>\n"
...
def debug_ai_config() -> None:
    ...
    print(
        ...
        f"normal_timeout={AI_READ_TIMEOUT}s claude_timeout={AI_CLAUDE_READ_TIMEOUT}s "
        f"claude_retries={AI_CLAUDE_MAX_REQUESTS} compact_claude={AI_USE_COMPACT_CLAUDE_AUDIT} "
...
def _phase_system(ai_name: str) -> str:
    base = "只输出 JSON 数组。禁止 markdown、禁止 JSON 外说明。不得引用 CRS、本地矩阵、贝叶斯、陷阱编号或固定常见比分模板。"
    if ai_name == "gpt":
...
async def async_call_one_ai_batch(
    ...
    max_requests = AI_MAX_REQUESTS_PER_AI
    if ai_name == "claude":
        max_requests = max(AI_MAX_REQUESTS_PER_AI, AI_CLAUDE_MAX_REQUESTS)

    for req_no in range(1, max_requests + 1):
        AI_CALL_STATUS[ai_name]["requests"] = req_no
        gateway = url.split("/v1")[0][:80]
        print(f"  [连接中] {ai_name.upper()} | {model} @ {gateway} | request#{req_no} | prompt={len(prompt):,} chars")
        t0 = time.time()
        ...
        try:
            if ai_name == "claude":
                timeout = aiohttp.ClientTimeout(
                    total=None,
                    connect=AI_CLAUDE_CONNECT_TIMEOUT,
                    sock_connect=AI_CLAUDE_CONNECT_TIMEOUT,
                    sock_read=AI_CLAUDE_READ_TIMEOUT,
                )
            else:
                timeout = aiohttp.ClientTimeout(
                    total=None,
            ...
            async with session.post(url, headers=headers, json=payload, timeout=timeout) as r:
                ...
                if r.status != 200:
                    try:
                        ...
                        text_for_error = ""
                    print(f"    HTTP {r.status} | {elapsed}s | {text_for_error[:240]}")
                    AI_CALL_STATUS[ai_name].update({"status": f"http_{r.status}", "elapsed": elapsed})
                    if ai_name == "claude" and req_no < max_requests:
                        await asyncio.sleep(AI_CLAUDE_RETRY_BACKOFF_SECONDS * req_no)
                    continue
                ...
                if parsed:
                    ...
                    print(f"    {ai_name.upper()} 完成: {len(parsed)}/{num_matches} | {total_elapsed}s")
            ...
            print(f"    {ai_name.upper()} 读取超时 request#{req_no}")
            AI_CALL_STATUS[ai_name].update({"status": "timeout"})
            if ai_name == "claude" and req_no < max_requests:
                await asyncio.sleep(AI_CLAUDE_RETRY_BACKOFF_SECONDS * req_no)
        except (aiohttp.ClientOSError, aiohttp.ServerDisconnectedError, aiohttp.ClientPayloadError, ConnectionResetError) as e:
            print(f"    {ai_name.upper()} 连接中断/重置 request#{req_no}: {str(e)[:180]}")
            AI_CALL_STATUS[ai_name].update({"status": "connection_reset", "error": str(e)[:300]})
            if ai_name == "claude" and req_no < max_requests:
                await asyncio.sleep(AI_CLAUDE_RETRY_BACKOFF_SECONDS * req_no)
        except Exception as e:
            print(f"    {ai_name.upper()} 调用异常 request#{req_no}: {str(e)[:180]}")
...
def _phase1_valid_model_count(all_results: Dict[str, Dict[int, Dict[str, Any]]]) -> int:
    return sum(1 for n in PHASE1_NAMES if bool(all_results.get(n)))
...
async def _run_ai_matrix_two_phase_inner(match_analyses: List[Dict[str, Any]], cache_key: str) -> Dict[str, Dict[int, Dict[str, Any]]]:
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        valid_phase1 = _phase1_valid_model_count(all_results)
        if AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID and valid_phase1 < AI_MIN_PHASE1_VALID_FOR_CLAUDE:
            print(f"  [Claude Skip] Phase1有效模型={valid_phase1} < {AI_MIN_PHASE1_VALID_FOR_CLAUDE}，跳过Claude避免白跑")
            all_results["claude"] = {}
        else:
            audit_prompt = build_claude_final_audit_prompt(match_analyses, all_results)
            print(f"  [v18.4 Phase2 Claude Audit] {len(audit_prompt):,}字符 | compact={AI_USE_COMPACT_CLAUDE_AUDIT} | timeout={AI_CLAUDE_READ_TIMEOUT}s")
...
def _make_ai_prediction(
    ...
    h_score, a_score = _parse_score(score)
    ...
    return {
        ...
        **selection_pack,
        ...
        "both_score": "是" if (h_score or 0) > 0 and (a_score or 0) > 0 else "否",
