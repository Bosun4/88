...
def _web_research_instruction(role: str) -> str:
    ...
    if role == "grok":
        return common + "你的联网重点：最新伤停、预计首发、临场新闻、资金流/热度/市场异动，尤其是临场资金时间序列。" + MARKET_TIMELINE_RERANK_ADDENDUM
    if role == "gemini":
...
def adapt_ai_to_frontend(ai_r: Dict[str, Any], match_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "market_timeline_audit": ai_r.get("market_timeline_audit", {}),
...
【临场资金时间序列升降级协议：证据闸版】
目标：只在存在真实多时间点盘口证据时，允许把比分从静态低赔锚升级为“多时间点 + 多公司 + 多市场闭环”的动态读盘；禁止泊松，禁止机械锚定最低赔比分。
1. 必须输出 market_timeline_audit：available、timeline_unavailable、missing_timeline_points、timeline_points_seen、bookmakers_seen、markets_seen、late_money_role、rerank_applied、upgraded_scores、downgraded_scores、minimum_evidence_needed、why_this_can_fail。
2. 时间点：若具备联网或外部盘口能力，必须尽量收集 初盘 / T-24h / T-6h / T-90m / T-60m / T-30m / 临场最终；缺口必须写 missing_timeline_points，不得编造。
3. 公司角色：至少尝试区分 百家平均、竞彩官方、威廉、澳门、Bet365、Pinnacle、立博。Pinnacle/Bet365/澳门的亚盘与大小球最后变化优先级高于早盘正确比分低赔；竞彩官方用于识别区域风控与大众入口。
4. 四市场闭环：欧赔负责方向与分流，亚洲让球负责赢球深度/不败保护，大小球负责总进球边界，BTTS/零封/球队进球数负责双方进球结构，正确比分只做最后映射，不得单独主导。
5. 聪明钱联动：最后 T-60m/T-30m 若出现跨市场同步，必须判断它是确认、诱导、分流还是回补，并说明对应比分升降级。
6. 比分升降级：低分主线遇到主队低水保护 + 大球水下压、小球水抬升时，才可把 2-1/1-2 等 3球边界比分从尾部升权；让球升深 + 大小压低 + BTTS/零封一致时，才可把 2-0/0-2/3-0/0-3 升权而不是追 2-1。
7. 硬闸：无真实时间序列不得升权。若没有真实时间序列数据，必须写 timeline_unavailable=true、rerank_applied=false，并降级信心；不得只凭静态正确比分、总进球簇或主观复盘把比分升权。
8. 反证条件：每场必须写 minimum_evidence_needed / why_this_can_fail；明确哪些 T-30m 盘口/水位变化会推翻当前主线，哪些比分因此降级或升权。
""".strip()
...
def build_phase1_prompt(evidence_batch: List[Dict[str, Any]], ai_name: str) -> str:
    ...
    p.append(MARKET_TIMELINE_RERANK_ADDENDUM)
    if ai_name == "gpt":
...
def apply_market_timeline_rerank_gate(pred: Dict[str, Any]) -> Dict[str, Any]:
    """Only recommendation-gate unsupported timeline reranks; never rewrite AI score/direction."""
    if not isinstance(pred, dict):
        return pred
    audit = pred.get("market_timeline_audit", {}) if isinstance(pred.get("market_timeline_audit"), dict) else {}
    if not audit:
        return pred
    rerank_applied = bool(audit.get("rerank_applied"))
    unavailable = bool(audit.get("timeline_unavailable")) or not bool(audit.get("available"))
    points = audit.get("timeline_points_seen", [])
    if not isinstance(points, list):
        points = [str(points)] if points else []
    late_points = {str(x).lower() for x in points}
    lacks_late_confirmation = not any("t-60" in x or "t-30" in x or "临场" in x for x in late_points)
    if rerank_applied and (unavailable or lacks_late_confirmation):
        _cap_recommendation_tier(pred, "D", "timeline_rerank_without_real_timeline", "timeline_unavailable")
        pred.setdefault("recommendation_downgrade_reasons", []).append("timeline_rerank_without_real_timeline")
        rec = pred.setdefault("recommendation", {}) if isinstance(pred.get("recommendation"), dict) else {}
        pred["recommendation"] = rec
        rec["tier"] = "D"
        rec["is_recommended"] = False
        rec["bet_action"] = "no_bet"
        rec["risk_level"] = "high"
        pred["risk_level"] = "high"
        pred["recommendation_tier"] = "D"
        pred["recommend_gate_pass"] = False
        pred.setdefault("recommend_gate_reasons", []).append("timeline_rerank_without_real_timeline")
        pred.setdefault("validation_warnings", []).append("timeline_unavailable_rerank_blocked")
        if _f(pred.get("display_confidence", pred.get("confidence", 0)), 0) > 49:
            pred["display_confidence"] = 49
        if _f(pred.get("risk_adjusted_confidence", pred.get("confidence", 0)), 0) > 49:
            pred["risk_adjusted_confidence"] = 49
...
def adapt_ai_to_frontend(ai_r: Dict[str, Any], match_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k in [
        ...
        "goal_market_audit", "market_timeline_audit", "market_conflicts", "candidate_scores", "public_heat_audit",
    ...
    try:
        ...
        apply_market_timeline_rerank_gate(pred)
        rec = pred.get("recommendation", {}) if isinstance(pred.get("recommendation"), dict) else {}
        tier = str(rec.get("tier", "D")).upper()
        pred["recommendation_tier"] = tier
        pred["recommend_gate_pass"] = bool(rec.get("is_recommended", False)) and _min_tier_ok(tier)
