...
def get_clean_env_url(name, default=""):
    ...
    if FORCE_COMMON_GATEWAY_URL:
        common = get_first_clean_env_url(COMMON_GATEWAY_URL_ALIASES, "")
        if common:
            return common
...
def get_clean_env_key(name):
    ...
    """
    if FORCE_COMMON_GATEWAY_URL:
        common = get_first_clean_env_key(COMMON_GATEWAY_KEY_ALIASES, "")
        if common:
            return common
...
def debug_ai_config():
    ...
    print(f"[COMMON GATEWAY] force={FORCE_COMMON_GATEWAY_URL} API_URL={get_first_clean_env_url(COMMON_GATEWAY_URL_ALIASES, '')} API_KEY={_mask_key(get_first_clean_env_key(COMMON_GATEWAY_KEY_ALIASES, ''))}")
...
async def _run_ai_matrix_two_phase_inner(match_analyses, prompt: str, cache_key: str):
    ...
    if not AI_DISABLE_CACHE and ok_count > 0:
        ...
        print("  [AI CACHE] 本轮 0/4 AI 有效，不写入缓存，避免失败结果污染后续批次")
    return all_results
