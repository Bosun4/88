# -*- coding: utf-8 -*-
"""
vMAX 18.4.7 — PURE RAW-AI + 历史经验审计卡 + 4场小批次顺序执行版
====================================================================
设计边界：
...
8. singleflight + 内存缓存 + 持久化磁盘缓存 + 磁盘锁：同批次重复触发时不重复扣费；但 0/4 AI 失败不写缓存。
...
11. v18.4.7 新增：AI_BATCH_SIZE 小批次；默认 4场一批；默认每批内 GPT→Grok→Gemini 顺序执行；Claude 可按 Phase1 有效数条件触发；Claude 终审可压缩 prompt。
...
ENGINE_ARCHITECTURE = (
    "PURE RAW-AI: 完整抓包raw_match_full_json + 体彩竞彩赔率标注 + AI自主联网核对欧洲主流欧赔 "
    "+ Sharp/聪明钱方向识别 + 联赛/球队风格比分约束 + AI direction_probs优先 "
    "+ 历史经验审计卡强制回应 + 四层比分结构(direction/goal_band/btts/score) "
    "+ 方向精选分/比分形态分双评分 + GPT/Grok/Gemini初审 + Claude终审；"
    "无CRS/无本地矩阵/AI失败即弃权 + 4场小批次 + 默认顺序调用 + 持久化缓存/磁盘锁防重复扣费"
)
...
# v18.4.7: 小批次 + 按量Key成本控制。
AI_BATCH_SIZE = max(1, _env_int("AI_BATCH_SIZE", 4))
AI_BATCH_CONCURRENCY = max(1, _env_int("AI_BATCH_CONCURRENCY", 1))
AI_MODEL_CONCURRENCY = max(1, _env_int("AI_MODEL_CONCURRENCY", 1))
AI_PHASE1_PARALLEL = _env_bool("AI_PHASE1_PARALLEL", False)
AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID = _env_bool("AI_RUN_CLAUDE_ONLY_IF_PHASE1_VALID", True)
AI_MIN_PHASE1_VALID_FOR_CLAUDE = max(1, _env_int("AI_MIN_PHASE1_VALID_FOR_CLAUDE", 2))
...
CLAUDE_AUDIT_RAW_PACKET_CHAR_LIMIT = max(1500, _env_int("CLAUDE_AUDIT_RAW_PACKET_CHAR_LIMIT", 8000))

# 抓包输入完整度控制。默认把 normalized match 的完整 JSON 喂给 AI。
...
# 持久化缓存 + 磁盘锁，防止 GitHub Actions / 前端重复触发时重复扣费。
...
# 历史经验规则迁移为“AI审计卡”。只进入 prompt 提问，不改概率、不改方向、不改比分。
...
async def build_external_context_for_match(session: aiohttp.ClientSession, match_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bing_key:
        ...
        q = f"{h} {a} {league} injury lineup weather football odds"
...
class ExperienceAuditEngine:
    ...
    def analyze(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if EXPERIENCE_AUDIT_INCLUDE_EXTENDED:
            ...
            if (0 < a1_val <= 5.2):
                add("X03", "二次升级", 6, f"1球@{a1_val}，需审计经济型赢球与BTTS", "audit_goal_shape")
...
def _prompt_header() -> str:
    ...
    p += "你必须结合联赛风格和球队风格给出比分形态：强队控场型、反击型、低节奏防守型、高节奏英超型、杯赛保守型会对应不同进球分布。\n"
    ...
    p += "7. 不要默认输出 1-1、2-1、1-0、0-1、1-2；若选择这些比分，必须是从原始字段、Sharp方向、联赛/球队风格和联网核对中独立推导出的结果。\n"
    p += '8. top3 必须是比分分布，不要只给方向。top3[0].score 必须与 final_direction 一致。必须额外输出 direction_probs={"home":数字,"draw":数字,"away":数字}。\n'
    ...
    p += "需要核对：欧洲主胜/平/客胜大致区间、盘口方向、赔率变化是否与体彩竞彩同向、是否存在中外市场背离。禁止编造具体公司、赔率或新闻。\n"
    p += "</web_odds_search_instruction>\n\n"
    ...
    p += "必须把联赛风格与球队风格转化为比分形态，而不是只判断方向。需要考虑：联赛平均节奏、爆冷率、平局倾向、强弱队差距、球队控球/压迫/反击/防守风格、近期进失球、杯赛或联赛战意。\n"
    p += "如果你联网得到球队风格或联赛风格资料，写入 audit.league_style 与 audit.team_style；如果没有资料，用原始赔率和已给情报推断，并说明不确定性。\n"
    p += "</league_team_style_instruction>\n\n"

    p += "<output_format>\n"
    p += "严格输出 JSON 数组。每场一个对象：\n"
    p += '{"match":1,"final_direction":"home/draw/away","direction_probs":{"home":45,"draw":28,"away":27},"goal_band":"0-1/2/3/4+","btts":"yes/no/unclear","top3":[{"score":"2-0","prob":0.18,"market_logic":"..."}],"reason":"...","ai_confidence":0-100,"risk_level":"low/medium/high","data_missing":[],"score_shape_reason":"...","audit":{"odds_source":"体彩竞彩抓包赔率","web_odds_check":"searched/web_search_unavailable/european_odds_missing","european_odds":"...","market_divergence":"...","sharp_money_direction":"home/draw/away/home_or_draw/away_or_draw/unclear","sharp_evidence":"...","league_style":"...","team_style":"...","style_score_logic":"...","direction_rejection":"...","experience_review":"EXP_D01:neutral because ...; EXP_X01:accepted because ..."}}\n'
    ...
    for i, ma in enumerate(match_analyses, 1):
        ...
        p += _raw_field_line(
            ...
            {k: v for k, v in m.items() if k in (
                ...
                "lineup", "news", "motivation", "schedule",
            )},
...
def build_claude_final_audit_prompt(match_analyses: List[Dict[str, Any]], phase1_results: Dict[str, Dict[int, Dict[str, Any]]]) -> str:
    ...
    p += "match_data 中的赔率是体彩竞彩赔率；欧洲主流欧赔需要你自己联网核对。若无联网能力，必须写 web_search_unavailable，不能假装查过。\n"
    p += "experience_audit_cards 只代表历史经验审计问题，不是本地结论；你必须在 audit.experience_review 中逐条说明 accepted/rejected/neutral，不得照单全收。\n"
...
def build_claude_final_audit_prompt_compact(
    match_analyses: List[Dict[str, Any]],
    phase1_results: Dict[str, Dict[int, Dict[str, Any]]],
) -> str:
    p = "<final_audit_context>\n"
    p += "你是 Claude 最终 RAW-AI 主裁。你看到的是中国体彩竞彩抓包赔率、原始比赛字段、历史经验审计卡，以及 GPT/Grok/Gemini 初审摘要。\n"
    p += "你不是反指模型，不需要为了体现审计而反对初审。选择证据最完整、最符合体彩竞彩原始字段、联网欧赔核对、Sharp/聪明钱方向、联赛/球队风格的一组。\n"
    p += "match_data 中的赔率是体彩竞彩赔率，不是欧洲平均欧赔。若你有联网能力，必须核对欧洲主流欧赔；若没有联网能力，写 web_search_unavailable，禁止假装查过。\n"
    p += "experience_audit_cards 只代表旧版历史经验审计问题，不是本地裁决。你必须在 audit.experience_review 中说明 accepted/rejected/neutral。\n"
    p += "必须输出四层比分结构：final_direction、direction_probs、goal_band、btts、score_shape_reason。比分必须与 goal_band / BTTS 自洽。\n"
    p += "禁止引用 CRS、贝叶斯、本地矩阵、本地风控编号、固定常见比分模板。\n"
    p += "如果你改动初审比分，必须给出硬依据，不能只是把 2-0 改成 2-1 或把 3-1 改成 2-1。\n"
    p += "严格输出 JSON 数组，禁止 JSON 外文本。\n"
    p += "</final_audit_context>\n\n"

    p += "<output_format>\n"
    p += '{"match":1,"final_direction":"home/draw/away","direction_probs":{"home":45,"draw":28,"away":27},"goal_band":"0-1/2/3/4+","btts":"yes/no/unclear","top3":[{"score":"2-0","prob":0.18,"market_logic":"..."}],"reason":"...","ai_confidence":0-100,"risk_level":"low/medium/high","data_missing":[],"score_shape_reason":"...","audit":{"odds_source":"体彩竞彩抓包赔率","web_odds_check":"searched/web_search_unavailable/european_odds_missing","european_odds":"...","market_divergence":"...","sharp_money_direction":"home/draw/away/home_or_draw/away_or_draw/unclear","sharp_evidence":"...","league_style":"...","team_style":"...","style_score_logic":"...","direction_rejection":"...","experience_review":"EXP_X01:accepted because ...; EXP_D01:neutral because ..."}}\n'
    p += "</output_format>\n\n"
    ...
    for i, ma in enumerate(match_analyses, 1):
        ...
        p += _raw_field_line("体彩竞彩赔率变动抓包字段", m.get("change"), FIELD_LIMIT_CHANGE)
        p += _raw_field_line("体彩竞彩散户/热度抓包字段", m.get("vote"), FIELD_LIMIT_VOTE)
        p += _raw_field_line("information原始字段", m.get("information"), min(FIELD_LIMIT_INFORMATION, 2500))
        p += _raw_field_line("points原始字段", m.get("points"), min(FIELD_LIMIT_POINTS, 2500))
        if INCLUDE_FULL_RAW_PACKET:
            raw_json = _json_compact(m, CLAUDE_AUDIT_RAW_PACKET_CHAR_LIMIT)
            p += f"raw_match_full_json:{raw_json}\n"
        exp = ma.get("experience_audit") or _experience_engine().analyze(m)
        p += _format_experience_audit_for_prompt(exp)
        p += "<external_context>\n" + _format_external_context_for_prompt(ma.get("external_context", {})) + "</external_context>\n"
        p += "</match>\n\n"
    ...
    for ai in PHASE1_NAMES:
        ...
        for idx in range(1, len(match_analyses) + 1):
            ...
            if r:
                ...
                audit_compact = {
                    ...
                    "european_odds": str(audit.get("european_odds", ""))[:300],
                    "market_divergence": str(audit.get("market_divergence", ""))[:300],
                    "sharp_money_direction": audit.get("sharp_money_direction", ""),
                    "sharp_evidence": str(audit.get("sharp_evidence", ""))[:300],
                    "league_style": str(audit.get("league_style", ""))[:250],
                    "team_style": str(audit.get("team_style", ""))[:250],
                    "experience_review": str(audit.get("experience_review", ""))[:500],
                }
                p += json.dumps({
                    "match": idx,
                    "ai_score": r.get("ai_score"),
        ...
        p += f"</{ai}>\n"
    p += "</phase1_ai_results_compact>\n\n"
    p += "<final_output_rule>严格输出 JSON 数组，字段同 output_format。禁止 JSON 外文本。</final_output_rule>\n"
    return p
...
def debug_ai_config() -> None:
    ...
    print(
        ...
        f"batch_size={AI_BATCH_SIZE} batch_concurrency={AI_BATCH_CONCURRENCY} "
        f"model_concurrency={AI_MODEL_CONCURRENCY} phase1_parallel={AI_PHASE1_PARALLEL}"
    )
...
async def _run_ai_matrix_two_phase_inner(
    ...
    print(
        f"  [v18.4 Phase1 Prompt] {len(prompt):,}字符 → GPT/Grok/Gemini "
        f"| common_gateway={bool(get_common_key() and get_common_url())} "
        f"| phase1_parallel={AI_PHASE1_PARALLEL} "
        f"| model_concurrency={AI_MODEL_CONCURRENCY}"
    )

    all_results: Dict[str, Dict[int, Dict[str, Any]]] = {n: {} for n in AI_NAMES}
    connector = aiohttp.TCPConnector(limit=max(2, AI_MODEL_CONCURRENCY + 1), use_dns_cache=False)

    async with aiohttp.ClientSession(connector=connector) as session:
        if not AI_PHASE1_PARALLEL or AI_MODEL_CONCURRENCY <= 1:
            for name in PHASE1_NAMES:
                try:
                    res = await async_call_one_ai_batch(
                        session=session,
                        prompt=prompt,
                        num_matches=num,
                        ai_name=name,
                        system_text=_phase_system(name),
                    )
                    if isinstance(res, tuple):
                        all_results[res[0]] = res[1]
                except Exception as e:
                    print(f"  [Phase1 {name.upper()} ERROR] {str(e)[:160]}")
        else:
            sem = asyncio.Semaphore(AI_MODEL_CONCURRENCY)

            async def _run_phase1_one(name: str):
                async with sem:
                    return await async_call_one_ai_batch(
                        session=session,
                        prompt=prompt,
                        num_matches=num,
                        ai_name=name,
            ...
            tasks = [_run_phase1_one(name) for name in PHASE1_NAMES]
        ...
        phase1_valid = sum(1 for name in PHASE1_NAMES if all_results.get(name))
        print(
            f"  [Phase1完成] valid={phase1_valid}/3 | "
            f"GPT={len(all_results.get('gpt', {}) or {})}/{num} "
            f"GROK={len(all_results.get('grok', {}) or {})}/{num} "
            f"GEMINI={len(all_results.get('gemini', {}) or {})}/{num}"
        )
        ...
        if should_run_claude:
            if AI_USE_COMPACT_CLAUDE_AUDIT:
                audit_prompt = build_claude_final_audit_prompt_compact(match_analyses, all_results)
            else:
                audit_prompt = build_claude_final_audit_prompt(match_analyses, all_results)

            print(
                f"  [v18.4 Phase2 Claude Audit] {len(audit_prompt):,}字符 "
                f"| compact={AI_USE_COMPACT_CLAUDE_AUDIT}"
            )

            try:
                _, cl_res, _ = await async_call_one_ai_batch(
                    session=session,
                    prompt=audit_prompt,
                    num_matches=num,
                    ai_name="claude",
                    system_text=_phase_system("claude"),
                )
                all_results["claude"] = cl_res or {}
            except Exception as e:
                print(f"  [Claude ERROR] {str(e)[:160]}")
                all_results["claude"] = {}
        else:
            print(
                f"  [Claude SKIP] Phase1有效模型 {phase1_valid}/3 < "
...
async def run_ai_matrix_batched(
    match_analyses: List[Dict[str, Any]],
) -> Dict[str, Dict[int, Dict[str, Any]]]:
    ...
    print(
        f"  [AI BATCH MODE] total={total} | "
        f"batch_size={batch_size} | "
        f"batch_concurrency={AI_BATCH_CONCURRENCY} | "
        f"model_concurrency={AI_MODEL_CONCURRENCY} | "
        f"phase1_parallel={AI_PHASE1_PARALLEL}"
    )

    async def _run_one_batch(start_idx: int, batch: List[Dict[str, Any]]):
        end_idx = start_idx + len(batch)
        print("\n" + "-" * 76)
        print(f"  [AI BATCH START] 全局场次 {start_idx + 1}-{end_idx}/{total}")
        print("-" * 76)

        try:
            batch_ai = await run_ai_matrix_two_phase(batch)
        except Exception as e:
            logger.error(f"AI batch {start_idx + 1}-{end_idx} 执行失败: {e}")
            batch_ai = {n: {} for n in AI_NAMES}

        mapped: Dict[str, Dict[int, Dict[str, Any]]] = {n: {} for n in AI_NAMES}

        for model_name in AI_NAMES:
            result_map = batch_ai.get(model_name, {}) or {}
            for local_mid, result in result_map.items():
                try:
                    local_mid_int = int(local_mid)
                except Exception:
                    continue
                global_mid = start_idx + local_mid_int
                if 1 <= global_mid <= total:
                    mapped[model_name][global_mid] = result

        print(
            f"  [AI BATCH DONE] 全局场次 {start_idx + 1}-{end_idx}/{total} | "
            f"GPT={len(mapped.get('gpt', {}) or {})} "
            f"GROK={len(mapped.get('grok', {}) or {})} "
            f"GEMINI={len(mapped.get('gemini', {}) or {})} "
            f"CLAUDE={len(mapped.get('claude', {}) or {})}"
        )
        return mapped

    batches = list(_chunk_list(match_analyses, batch_size))

    if AI_BATCH_CONCURRENCY <= 1:
        for start_idx, batch in batches:
            mapped = await _run_one_batch(start_idx, batch)
            for model_name in AI_NAMES:
                all_ai[model_name].update(mapped.get(model_name, {}) or {})
    else:
        sem = asyncio.Semaphore(AI_BATCH_CONCURRENCY)

        async def _guarded(start_idx: int, batch: List[Dict[str, Any]]):
            async with sem:
                return await _run_one_batch(start_idx, batch)

        results = await asyncio.gather(
            *[_guarded(start_idx, batch) for start_idx, batch in batches],
            return_exceptions=True,
        )
        for mapped in results:
            if isinstance(mapped, Exception):
                logger.error(f"AI batch 并发异常: {mapped}")
                continue
            for model_name in AI_NAMES:
                all_ai[model_name].update(mapped.get(model_name, {}) or {})
    ...
    print(
        f"\n  [AI BATCH ALL DONE] "
        f"GPT={len(all_ai['gpt'])}/{total} "
        f"GROK={len(all_ai['grok'])}/{total} "
        f"GEMINI={len(all_ai['gemini'])}/{total} "
        f"CLAUDE={len(all_ai['claude'])}/{total}"
    )
    return all_ai
...
def _abstain_prediction(reason: str = "AI全失败，PURE模式不使用本地兜底") -> Dict[str, Any]:
    return {
        ...
        "btts_ai": "unclear",
        "score_shape_reason": "",
        "direction_selection_score": 0,
        "score_shape_score": 0,
        "overall_selection_score": 0,
        ...
        "experience_analysis": {"mode": "prompt_only_no_decision", "enabled": ENABLE_EXPERIENCE_AUDIT_CARDS, "triggered_count": 0, "total_score": 0, "rules": []},
...
def _make_ai_prediction(
    ...
    return {
        ...
        **selection_pack,
        ...
        "kelly_home": {}, "kelly_away": {}, "odds": {}, "experience_analysis": {"mode": "prompt_only_no_decision", **exp_audit}, "pro_odds": {},
...
def select_top4(preds: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    for p in preds:
        ...
        if pr.get("is_abstain"):
            ...
            continue
        s = _f(pr.get("overall_selection_score", 0), 0)
        if s <= 0:
            s = _f(pr.get("confidence", 0), 0) + _f(pr.get("dir_confidence", 0), 0) * 0.2
...
def run_predictions(raw: Dict[str, Any], use_ai: bool = True):
    ...
    if use_ai and match_analyses:
        ...
        try:
            all_ai = _run_async(run_ai_matrix_batched(match_analyses))
        except Exception as e:
    ...
    for i, ma in enumerate(match_analyses, 1):
        ...
        if mg.get("is_abstain"):
            ...
            print(
                ...
                f"AI_CF:{mg['confidence']} | 推荐层:{mg.get('recommendation_tier')} | 来源:{mg.get('decision_source')}"
            )
...
if __name__ == "__main__":
    ...
    print("   模式: AI成功=AI直出；AI失败=弃权；无本地兜底；默认4场小批次顺序执行")
