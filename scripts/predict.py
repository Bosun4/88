...
p = “”
p += “[SYSTEM] You are a cold-blooded football quant risk officer.\n”
p += “[TASK] Independent match verdict for [%s] vs [%s].\n” % (h, a)
p += “[RULES] No fabrication. No player names. Pure JSON only. No Markdown.\n\n”
p += “[MATCH] %s | %s\n” % (lg, m.get(“match_num”, “?”))
p += “[SP ODDS] H %s D %s A %s | Handicap: %s\n” % (sp_h, sp_d, sp_a, m.get(“give_ball”, “?”))
p += “[MOVEMENT] %s\n” % m.get(“odds_movement”, “unknown”)
p += “\n[HOME %s] Rank #%s\n” % (h, m.get(“home_rank”, “?”))
p += “Record: %sP %sW %sD %sL GF:%s GA:%s AvgGF:%s AvgGA:%s\n” % (
...
p += “Form: %s | CS: %s\n” % (hs.get(“form”,”?”), hs.get(“clean_sheets”,”?”))
p += “Injuries: %s\n” % intel.get(“h_inj”, “unknown”)
p += “\n[AWAY %s] Rank #%s\n” % (a, m.get(“away_rank”, “?”))
p += “Record: %sP %sW %sD %sL GF:%s GA:%s AvgGF:%s AvgGA:%s\n” % (
...
p += “Form: %s | CS: %s\n” % (ast.get(“form”,”?”), ast.get(“clean_sheets”,”?”))
p += “Injuries: %s\n” % intel.get(“g_inj”, “unknown”)
h2h = m.get(“h2h”, [])
if h2h:
p += “\n[H2H]\n”
for x in h2h[:5]:
...
p += “  %s odds=%.2f implied=%.1f%%\n” % (sc, od, prob)
p += “\n[DECISION RULES]\n”
p += “1. CoT: calc SP implied prob -> compare public vote -> home advantage +0.4g -> injury impact -> form -> H2H -> league tendency\n”
p += “2. Anchor score on CRS TOP1 or TOP2, then adjust with data\n”
p += “3. Public >55%% but odds stable = trap, think reverse\n”
p += “4. Home hot + shallow handicap = upset risk\n”
p += “5. Give 3 evidence + 1 counter-argument\n”
p += “6. Exact score only\n\n”
p += “[OUTPUT] Pure JSON only:\n”
p += ‘{“ai_score”:“1-2”,“analysis”:“80 char cold analysis with 3 evidence + 1 rebuttal”}’
return p
...
sp_h, sp_d, sp_a = m.get(“sp_home”, 0), m.get(“sp_draw”, 0), m.get(“sp_away”, 0)
p = “”
p += “[SYSTEM] Supreme Quant Arbiter. Your verdict is FINAL.\n”
p += “[TASK] Final score verdict for [%s] vs [%s].\n\n” % (h, a)
p += “[3-WAY RECON]\n”
...
p += “[STATS MATRIX]\n”
p += “Poisson: %s | Refined: %s\n” % (poi.get(“predicted_score”,”?”), ref.get(“predicted_score”,”?”))
p += “Fusion: H%.1f%% D%.1f%% A%.1f%%\n” % (
stats_pred.get(“home_win_pct”,33), stats_pred.get(“draw_pct”,33), stats_pred.get(“away_win_pct”,33))
p += “Consensus: %d/%d | xGoals: %.1f\n” % (
stats_pred.get(“model_consensus”,0), stats_pred.get(“total_models”,11),
stats_pred.get(“expected_total_goals”,2.5))
...
p += “\n[SP] H%s D%s A%s | HC: %s\n” % (sp_h, sp_d, sp_a, m.get(“give_ball”,”?”))
intel = m.get(“intelligence”, {})
p += “[INJ] H: %s\n” % str(intel.get(“h_inj”,”?”))[:80]
p += “      A: %s\n” % str(intel.get(“g_inj”,”?”))[:80]
...
p += “[FACE] %s\n” % baseface[:200]
p += “\n[VERDICT RULES]\n”
p += “1. Weight: AI consensus 50%% + Market TOP1 30%% + Stats 10%% + Injury/Signal 10%%\n”
p += “2. If 2+ AI agree on same score -> adopt directly (double weight)\n”
p += “3. Market TOP1 is the anchor unless ALL AI + stats strongly disagree\n”
p += “4. Consensus >=9/11 + market match -> lock immediately\n”
p += “5. Signal (mismatch/money flow) = institutional attitude, must override\n”
p += “6. Public landslide + odds stable = trap, reverse\n”
p += “7. One exact score. Explain why this beats other 3 AI scores.\n\n”
p += “[OUTPUT] Pure JSON only:\n”
p += ‘{“ai_score”:“1-2”,“analysis”:“100 char final verdict with weight reasoning”}’
return p
...
for name, res in ai_sources:
    ...
    if score and score not in ["-", "?", ""]:
        ...
        w = 0.25 if name == "claude" else 0.18
...
has_warning = any("\u1f6a8" in str(sig) or "🚨" in str(sig) for sig in smart) or (extreme and extreme != "\u65e0")
...
agreement_bonus = 25 if len(set(ai_scores)) <= 1 and len(ai_scores) >= 2 else (15 if len(set(ai_scores)) <= 2 else 0)
...
ms = raw.get(“matches”, [])
print(”\n” + “=” * 60)
print(”  ENGINE v5.0 | %d matches” % len(ms))
print(”  AI weight 85%% | Stats 15%% | Multi-URL fallback”)
print(”=” * 60)
res = []
